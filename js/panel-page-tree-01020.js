...
export function initPageTreePanel() {
  ...
  function revealAndScrollTreeNode01020_(node) {
...
function updateCanvasSelection(scrollTargetEl = null) {
 ...
 function makeBlockNode(blockId, index, prefix) {
  ...
  document.addEventListener('st:selection-changed', (ev) => {
    ...
    // Global Design modes are a bulk selection. Their visual state in Tree is
    // owned by st:designSelectionModeChange; do not collapse it into one exact row.
    ...
    if (type === 'block') {
      ...
      if (treeVisible) {
        ...
        revealAndScrollTreeNode01020_(node);
      }
      return;
    }
