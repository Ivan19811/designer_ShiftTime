// 01056 · Builder appearance controls · main radius contract fix.
import { createBuilderUiPreferences01056 } from './builder-ui-preferences-01056.js?v=01056';

const STAGE = '01056';
const preferences = createBuilderUiPreferences01056();
...
const ADDITIONAL_COLOR_FIELDS = Object.freeze([
  ['additionalSurface','Фон додаткових блоків'],
  ['additionalSurfaceAlt','Фон чипів / бейджів'],
  ['additionalText','Основний текст блоків'],
  ['additionalMuted','Другорядний / службовий текст'],
  ['additionalBorder','Бордери додаткових блоків'],
  ['additionalAccent','Акцент / статуси']
]);

const SIDE_COLOR_FIELDS = Object.freeze([
  ['sidePanel','Фон бокової області'],
  ['sideSurface','Фон бокових блоків'],
  ['sideText','Основний текст бокових блоків'],
  ['sideMuted','Другорядний текст бокових блоків'],
  ['sideBorder','Бордери бокових блоків'],
  ['sideNotice','Фон інформаційного блока'],
  ['sideNoticeText','Текст інформаційного блока'],
  ['sideWarn','Фон попередження'],
  ['sideWarnText','Текст попередження']
]);

const ALL_COLOR_FIELDS = Object.freeze([...COLOR_FIELDS,...ADDITIONAL_COLOR_FIELDS,...SIDE_COLOR_FIELDS]);
...
function effectiveColors01054(state){
  ...
  const background=a.background||sampleColor01054('.mp-studio-01051','backgroundColor','#f4f7f5');
  const surface=a.surface||sampleColor01054('.mp-card','backgroundColor','#ffffff');
  const surfaceAlt=a.surfaceAlt||sampleColor01054('.mp-toolbar','backgroundColor','#f8fbf9');
  const header=a.header||sampleColor01054('.mp-topbar','backgroundColor','#ffffff');
  const text=a.text||sampleColor01054('.mp-page-head h1','color','#15231b');
  const muted=a.muted||sampleColor01054('.mp-page-head p','color','#6d7f75');
  const border=a.border||sampleColor01054('.mp-card','borderTopColor','#e0e9e4');
  const accent=a.accent||'#138a50';
  return {
    background,surface,surfaceAlt,header,text,muted,border,accent,
    additionalSurface:a.additionalSurface||surface,
    additionalSurfaceAlt:a.additionalSurfaceAlt||surfaceAlt,
    additionalText:a.additionalText||text,
    additionalMuted:a.additionalMuted||muted,
    additionalBorder:a.additionalBorder||border,
    additionalAccent:a.additionalAccent||accent,
    sidePanel:a.sidePanel||surfaceAlt,
    sideSurface:a.sideSurface||surface,
    sideText:a.sideText||text,
    sideMuted:a.sideMuted||muted,
    sideBorder:a.sideBorder||border,
    sideNotice:a.sideNotice||'#eef9f2',
    sideNoticeText:a.sideNoticeText||'#33714d',
    sideWarn:a.sideWarn||'#fff6e8',
    sideWarnText:a.sideWarnText||'#97620e'
  };
}

function hasMainStyleOverrides01056(a){
  if(!a)return false;
...
function hasAdditionalStyleOverrides01056(a){
  if(!a)return false;
  if(['additionalSurface','additionalSurfaceAlt','additionalText','additionalMuted','additionalBorder','additionalAccent'].some(k=>!!a[k]))return true;
  return ['additionalRadius','additionalBorderWidth'].some(k=>a[k]!==null&&a[k]!==undefined);
}
function hasSideStyleOverrides01056(a){
  if(!a)return false;
  if(['sidePanel','sideSurface','sideText','sideMuted','sideBorder','sideNotice','sideNoticeText','sideWarn','sideWarnText'].some(k=>!!a[k]))return true;
  return ['sideRadius','sideBorderWidth'].some(k=>a[k]!==null&&a[k]!==undefined);
}
...
function applyAppearance01054(state=preferences.get()){
  ...
  if(!root)return;
  const a=state.appearance||{};
  root.dataset.stUiCustom=hasMainStyleOverrides01056(a)?'1':'0';
  root.dataset.stUiAdditionalCustom=hasAdditionalStyleOverrides01056(a)?'1':'0';
  root.dataset.stUiSideCustom=hasSideStyleOverrides01056(a)?'1':'0';

  const mainScale=clamp01054(Number(a.fontScale)||1,.8,1.6);
  const additionalScale=clamp01054(Number(a.additionalFontScale)||1,.8,2.2);
  const sideScale=clamp01054(Number(a.sideFontScale)||1,.8,2.2);
  root.style.setProperty('--st-main-font-scale',String(mainScale));
  root.style.setProperty('--st-additional-font-scale',String(additionalScale));
  root.style.setProperty('--st-side-font-scale',String(sideScale));

  const mainSizes={8:8,9:9,10:10,11:11,12:12,14:14,15:15,17:17,19:19,23:23,25:25};
  Object.entries(mainSizes).forEach(([key,base])=>root.style.setProperty(`--st-main-font-${key}`,`${(base*mainScale).toFixed(2)}px`));
  const auxiliarySizes={8:8,9:9,10:10,11:11,12:12,15:15,17:17,23:23};
  Object.entries(auxiliarySizes).forEach(([key,base])=>root.style.setProperty(`--st-additional-font-${key}`,`${(base*mainScale*additionalScale).toFixed(2)}px`));
  const sideSizes={8:8,9:9,10:10,12:12};
  Object.entries(sideSizes).forEach(([key,base])=>root.style.setProperty(`--st-side-font-${key}`,`${(base*mainScale*sideScale).toFixed(2)}px`));
  ...
  const map={
    ...
    additionalSurface:'--st-ui-additional-surface',additionalSurfaceAlt:'--st-ui-additional-surface-alt',
    additionalText:'--st-ui-additional-text',additionalMuted:'--st-ui-additional-muted',
    additionalBorder:'--st-ui-additional-border',additionalAccent:'--st-ui-additional-accent',
    sidePanel:'--st-ui-side-panel',sideSurface:'--st-ui-side-surface',sideText:'--st-ui-side-text',sideMuted:'--st-ui-side-muted',
    sideBorder:'--st-ui-side-border',sideNotice:'--st-ui-side-notice',sideNoticeText:'--st-ui-side-notice-text',
    sideWarn:'--st-ui-side-warn',sideWarnText:'--st-ui-side-warn-text'
  };
  ...
  const numeric={
    radius:'--st-ui-radius',borderWidth:'--st-ui-border-width',workspacePadding:'--st-ui-workspace-padding',
    additionalRadius:'--st-ui-additional-radius',additionalBorderWidth:'--st-ui-additional-border-width',
    sideRadius:'--st-ui-side-radius',sideBorderWidth:'--st-ui-side-border-width'
  };
...
function appearanceHtml01054(){
  ...
  const renderColorRows=(fields)=>fields.map(([key,label])=>`<label class="st-ui-color-row"><span>${esc01054(label)}</span><div><input type="color" value="${esc01054(colors[key])}" data-st-ui-color="${key}"><input type="text" value="${esc01054(colors[key])}" maxlength="7" spellcheck="false" data-st-ui-color-text="${key}"></div></label>`).join('');
  const colorRows=renderColorRows(COLOR_FIELDS);
  const additionalColorRows=renderColorRows(ADDITIONAL_COLOR_FIELDS);
  const sideColorRows=renderColorRows(SIDE_COLOR_FIELDS);
    ...
    <div class="st-ui-settings-scroll">
      <section class="st-ui-settings-group"><div class="st-ui-settings-group-title"><b>Типографіка та геометрія</b><span>Основний масштаб і форма інтерфейсу.</span></div>
        <div class="st-ui-control-grid">
          ...
          <label class="st-ui-slider-control"><span>Радіус основних блоків <output data-st-ui-value="radius">${a.radius??16}px</output></span><input type="range" min="0" max="36" step="1" value="${a.radius??16}" data-st-ui-range="radius"></label>
      ...
      <section class="st-ui-settings-group"><div class="st-ui-settings-group-title"><b>Кольори</b><span>Основний фон, головні блоки, текст, бордери та акцент.</span></div><div class="st-ui-color-grid">${colorRows}</div></section>

      <section class="st-ui-settings-group st-ui-settings-group--extended">
        <div class="st-ui-settings-group-title"><b>Додаткові блоки та бокові елементи + текст</b><span>Окреме керування дрібними службовими блоками, статусами та правою боковою областю.</span></div>
        <div class="st-ui-subgroup-grid">
          <div class="st-ui-subgroup">
            <div class="st-ui-subgroup-title"><b>Додаткові блоки</b><span>«Останні операції», Discovery health, checklist, chips, badges та службові написи.</span></div>
            <div class="st-ui-control-grid">
              <label class="st-ui-slider-control"><span>Розмір тексту <output data-st-ui-value="additionalFontScale">${Math.round((Number(a.additionalFontScale)||1)*100)}%</output></span><input type="range" min="80" max="220" step="5" value="${Math.round((Number(a.additionalFontScale)||1)*100)}" data-st-ui-range="additionalFontScale"></label>
              <label class="st-ui-slider-control"><span>Радіус блоків <output data-st-ui-value="additionalRadius">${a.additionalRadius??17}px</output></span><input type="range" min="0" max="36" step="1" value="${a.additionalRadius??17}" data-st-ui-range="additionalRadius"></label>
              <label class="st-ui-slider-control is-wide"><span>Товщина бордера <output data-st-ui-value="additionalBorderWidth">${a.additionalBorderWidth??1}px</output></span><input type="range" min="0" max="4" step="1" value="${a.additionalBorderWidth??1}" data-st-ui-range="additionalBorderWidth"></label>
            </div>
            <div class="st-ui-color-grid st-ui-color-grid--single">${additionalColorRows}</div>
          </div>

          <div class="st-ui-subgroup">
            <div class="st-ui-subgroup-title"><b>Бокові елементи + текст</b><span>«Стан системи», «Джерела правди», Stub mode, інформаційні та warning-блоки.</span></div>
            <div class="st-ui-control-grid">
              <label class="st-ui-slider-control"><span>Розмір тексту <output data-st-ui-value="sideFontScale">${Math.round((Number(a.sideFontScale)||1)*100)}%</output></span><input type="range" min="80" max="220" step="5" value="${Math.round((Number(a.sideFontScale)||1)*100)}" data-st-ui-range="sideFontScale"></label>
              <label class="st-ui-slider-control"><span>Радіус блоків <output data-st-ui-value="sideRadius">${a.sideRadius??14}px</output></span><input type="range" min="0" max="36" step="1" value="${a.sideRadius??14}" data-st-ui-range="sideRadius"></label>
              <label class="st-ui-slider-control is-wide"><span>Товщина бордера <output data-st-ui-value="sideBorderWidth">${a.sideBorderWidth??1}px</output></span><input type="range" min="0" max="4" step="1" value="${a.sideBorderWidth??1}" data-st-ui-range="sideBorderWidth"></label>
            </div>
            <div class="st-ui-color-grid st-ui-color-grid--single">${sideColorRows}</div>
          </div>
        </div>
      </section>

      <section class="st-ui-settings-group st-ui-settings-note"><b>Збереження</b><p>Усі зміни, включно з додатковими та боковими блоками, зберігаються автоматично і відновлюються після F5, закриття браузера та перезавантаження комп’ютера. «Заводські» скидає тільки UI-настройки й не торкається товарів або MarketplaceStore.</p></section>
    </div>
...
function syncAppearanceControls01054(state=preferences.get()){
  ...
  const a=state.appearance||{};
  const values={
    fontScale:Math.round((Number(a.fontScale)||1)*100),radius:a.radius??16,borderWidth:a.borderWidth??1,shadow:a.shadow??45,workspacePadding:a.workspacePadding??18,
    additionalFontScale:Math.round((Number(a.additionalFontScale)||1)*100),additionalRadius:a.additionalRadius??17,additionalBorderWidth:a.additionalBorderWidth??1,
    sideFontScale:Math.round((Number(a.sideFontScale)||1)*100),sideRadius:a.sideRadius??14,sideBorderWidth:a.sideBorderWidth??1
  };
  const percentKeys=new Set(['fontScale','shadow','additionalFontScale','sideFontScale']);
  Object.entries(values).forEach(([key,value])=>{
    ...
    const output=appearanceWindow.querySelector(`[data-st-ui-value="${key}"]`);if(output)output.textContent=percentKeys.has(key)?`${value}%`:`${value}px`;
  });
  const colors=effectiveColors01054(state);
...
function commitRange01054(key, raw, persist=true){
  ...
  else if(key==='additionalFontScale')a.additionalFontScale=clamp01054(n,80,220)/100;
  else if(key==='sideFontScale')a.sideFontScale=clamp01054(n,80,220)/100;
  ...
  else if(key==='additionalRadius')a.additionalRadius=clamp01054(n,0,36);
  else if(key==='additionalBorderWidth')a.additionalBorderWidth=clamp01054(n,0,4);
  else if(key==='sideRadius')a.sideRadius=clamp01054(n,0,36);
  else if(key==='sideBorderWidth')a.sideBorderWidth=clamp01054(n,0,4);
  else return;
...
export function initBuilderUiAppearance01056(){
  applyInspector01054();
  applyAppearance01054();
  installButtonBindings01054();
  try{window.ST_BUILDER_UI_APPEARANCE_01056=Object.freeze({stage:STAGE,preferences,open:openAppearanceWindow01054,apply:()=>{applyInspector01054();applyAppearance01054();}});}catch{}
}
