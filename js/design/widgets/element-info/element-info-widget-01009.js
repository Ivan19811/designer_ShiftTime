// js/design/widgets/element-info/element-info-widget-01009.js
// 01009 — "Інформація елемента": readable inspector scale + Store/CSS authored parity.
...
const SECTION_ID = 'st-element-info-widget-01009';
const TOOLTIP_ID = 'st-element-info-help-01009';
const VERSION = '01009-element-info-readable-store-css-parity';
const FONT_SCALE_KEY = 'st_element_info_font_scale_v1';
const FONT_SCALE_DEFAULT = 130;
...
const COMPUTED_STYLE_KEYS_01009 = Object.freeze([
  'display','flex-direction','flex-wrap','align-items','align-self','justify-content','gap',
  'padding-top','padding-right','padding-bottom','padding-left',
  'background-image','background-color','color',
  ...
  'opacity','filter','overflow','overflow-x','overflow-y'
]);

function meaningfulComputedStyle01009(key, value, cs) {
  const v = String(value ?? '').trim();
  if (!v) return false;
  if (key === 'display') return /^(?:flex|inline-flex|grid|inline-grid|none)$/u.test(v);
  if (/^flex-|^align-|^justify-content$/u.test(key)) return /flex|grid/u.test(String(cs?.display || ''));
  if (key === 'gap') return v !== 'normal' && v !== '0px';
  if (key.startsWith('padding-')) return v !== '0px';
  if (key === 'background-image') return v !== 'none';
  if (key === 'background-color') return !/^(?:rgba?\(0, 0, 0, 0\)|transparent)$/iu.test(v);
  if (/^border-.*-width$/u.test(key)) return v !== '0px';
  if (/^border-.*-style$/u.test(key)) return !/^(?:none|hidden)$/u.test(v);
  if (/^border-.*-color$/u.test(key)) return !/^rgba?\(0, 0, 0, 0\)$/iu.test(v);
  if (key === 'border-radius') return v !== '0px';
  if (key === 'box-shadow' || key === 'filter' || key.includes('backdrop-filter')) return v !== 'none';
  if (key === 'opacity') return v !== '1';
  if (key.startsWith('overflow')) return v !== 'visible';
  // Typography is useful even when inherited: it explains what is actually visible.
  if (/^(?:color|font-size|font-weight|line-height|letter-spacing|text-align)$/u.test(key)) return true;
  return true;
}

function computedFallbackRows01009(element, occupiedKeys = new Set()) {
  if (!(element instanceof HTMLElement)) return [];
  let cs = null;
  try { cs = getComputedStyle(element); } catch (_) { return []; }
  const out = [];
  for (const key of COMPUTED_STYLE_KEYS_01009) {
    if (occupiedKeys.has(key)) continue;
    let value = '';
    try { value = String(cs.getPropertyValue(key) || '').trim(); } catch (_) {}
    if (!meaningfulComputedStyle01009(key, value, cs)) continue;
    out.push({ key, value, source:'css', authoredValue:null, baseValue:null, cssFallback:true });
  }
  return out;
}

function effectiveStyleRows(node, state, liveElement = null) {
  ...
  const occupied = new Set(rows.map(row => row.key));
  rows.push(...computedFallbackRows01009(liveElement, occupied));
...
function getNodeLabel(node, rt) {
  const explicit = node?.meta?.treeName || node?.meta?.name || node?.meta?.label || node?.meta?.dom?.dataset?.name || node?.content?.label || node?.content?.name;
...
function sourceText(source) {
  return ({ responsive:'Адаптив', json:'JSON', template:'Шаблон', css:'CSS' })[source] || 'JSON';
}
...
function jumpToWidget(map) {
  ...
  if (target instanceof HTMLElement) {
    target.classList.add('st-element-info-jump-01009');
    try { target.focus({ preventScroll:true }); } catch (_) {}
    setTimeout(() => target.classList.remove('st-element-info-jump-01009'), 1600);
  }
}
...
function makeTooltip() {
  ...
  tip.id = TOOLTIP_ID;
  tip.className = 'st-element-info-tooltip-01009';
  tip.hidden = true;
  tip.innerHTML = `
    <div class="st-element-info-tooltip-01009__eyebrow">SITEFRAMESTORE · ДЖЕРЕЛО ПРАВДИ</div>
    <div class="st-element-info-tooltip-01009__title">Інформація активного елемента</div>
    <div class="st-element-info-tooltip-01009__text">
      Клікніть на будь-яку секцію, рівень, контейнер, текст або інший елемент полотна. Тут буде показано саме його JSON: назву, ID, тип, усі Store-стилі, авторський CSS-fallback, адаптивні значення, геометрію та службовий контракт. Кружечок біля властивості відкриває відповідне місце інспектора, а олівець дозволяє змінити лише значення властивості. Під час редагування працює live-preview без Store/history; «Застосувати» робить один фінальний commit. Ключі, селектори та системні поля не редагуються.
    </div>`;
...
function renderSection(section, getSelection) {
  ...
  const previousPreviewId = String(section.__stElementInfoPreviewNodeId01009 || '');
  if (previousPreviewId && previousPreviewId !== nodeId) {
    try { rt?.renderElementInfoNode01008?.(previousPreviewId); } catch (_) {}
    section.__stElementInfoPreviewNodeId01009 = '';
  }
  ...
  const activeLiveElement01009 = liveOwner(node.id);
  const rows = effectiveStyleRows(node, state, activeLiveElement01009);
  ...
  const cssFallbackCount01009 = rows.filter(row => row.source === 'css').length;
  const jsonOwnedCount01009 = rows.length - cssFallbackCount01009;
  status.textContent = `${rows.length} видимих стилів · ${jsonOwnedCount01009} Store/шаблон · ${cssFallbackCount01009} CSS · ${counts.full} точних контролів`;
  const structural = {
  ...
  body.innerHTML = `
    ...
    <div class="stei-group">
      <div class="stei-group__title"><span>Стилі активного елемента · Store + CSS</span><b>${rows.length}</b></div>
      <div class="stei-style-list">${rows.length ? rows.map(rowMarkup).join('') : '<div class="stei-empty">Для цього елемента не знайдено ні Store-style, ні видимих CSS-стилів.</div>'}</div>
    </div>
...
function bindEditor(section, getSelection, node, row, editor) {
  ...
  function preview(next) {
    ...
    const el = owner();
    if (el) {
      section.__stElementInfoPreviewNodeId01009 = node.id;
...
export function initElementInfoWidget(host, getSelection) {
  ...
  section.className = 'design-section st-element-info-01009';
  section.dataset.elementInfoVersion = VERSION;
  section.innerHTML = `
    <button class="design-section__header" type="button" aria-expanded="false">
      <div class="design-section__header-title st-element-info-title-01009"><span>Інформація елемента</span><small>JSON · SiteFrameStore</small></div>
      <span class="design-section__chevron">▶</span>
    </button>
    <div class="design-section__body" hidden>
      <div class="stei-font-scale-01009">
        <div class="stei-font-scale-01009__head"><strong>Розмір шрифту віджета</strong><output data-ei-font-scale-value>${FONT_SCALE_DEFAULT}%</output></div>
        <input type="range" min="80" max="220" step="5" value="${FONT_SCALE_DEFAULT}" data-ei-font-scale aria-label="Розмір шрифту віджета Інформація елемента">
      </div>
  ...
  // 01009: local UI preference only. It never touches SiteFrameStore/history.
  const fontScaleInput01009 = section.querySelector('[data-ei-font-scale]');
  const fontScaleValue01009 = section.querySelector('[data-ei-font-scale-value]');
  const readFontScale01009 = () => {
    let value = FONT_SCALE_DEFAULT;
    try { value = Number(localStorage.getItem(FONT_SCALE_KEY) || FONT_SCALE_DEFAULT); } catch (_) {}
    if (!Number.isFinite(value)) value = FONT_SCALE_DEFAULT;
    return Math.max(80, Math.min(220, Math.round(value / 5) * 5));
  };
  const applyFontScale01009 = (raw, persist = false) => {
    let value = Number(raw);
    if (!Number.isFinite(value)) value = FONT_SCALE_DEFAULT;
    value = Math.max(80, Math.min(220, Math.round(value / 5) * 5));
    section.style.setProperty('--stei-font-scale', String(value / 100));
    if (fontScaleInput01009) fontScaleInput01009.value = String(value);
    if (fontScaleValue01009) fontScaleValue01009.textContent = `${value}%`;
    if (persist) { try { localStorage.setItem(FONT_SCALE_KEY, String(value)); } catch (_) {} }
  };
  applyFontScale01009(readFontScale01009(), false);
  fontScaleInput01009?.addEventListener('input', () => applyFontScale01009(fontScaleInput01009.value, true));

  // 3-second help tooltip on title hover. Pure UI; no observer/timer loop.
  const title = section.querySelector('.st-element-info-title-01009');
