...
// 00994: Fill adds independent element opacity + element blur while preserving the outer border.
...
<!-- Прозорість + чорно-білий -->
      <div class="design-field">
        ...
        <div class="design-subnote">
          «Прозорість фону» змінює тільки заливку. «Прозорість елемента» змінює весь вміст блока та його фон, але не зовнішню рамку. При 0% лишається тільки рамка. «Розмитість елемента» працює окремо; 5% = легке розмиття.
        </div>
      </div>
...
function getTargets() {
  ...
  function clearElementFx00994(target) {
    ...
    for (const prop of [ELEMENT_FX_SOURCE_VAR_00994,ELEMENT_FX_OWNS_BG_VAR_00994,'--st-element-fx-opacity','--st-element-fx-blur','--st-element-fx-blur-pct']) {
      target.style.removeProperty(prop);
    }
  }
