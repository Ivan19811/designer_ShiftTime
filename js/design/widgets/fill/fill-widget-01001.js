...
// 01001: Fill color/filter color hot paths are true single-authority live layers.
// Solid Fill uses only --st-bgfx-bg; solid Filter uses only --st-bgfx-filter-color.
// Native color-picker input is DOM-only RAF preview with one final Store commit.
...
<div class="design-field">
  ...
  function markFillAuthority00999(target) {
    if (target instanceof HTMLElement) target.style.setProperty(FILL_AUTHORITY_VERSION_VAR_00999, '01001');
  }
...
function getTargets() {
  ...
  function canonicalizeFilterAuthority00999(target, cs = null) {
    ...
    target.style.setProperty(FILL_AUTHORITY_VERSION_VAR_00999, '01001');
...
// --- УНІФІКОВАНИЙ РЕНДЕР ФОНУ ЧЕРЕЗ ::before (.st-bgfx) ---
  ...
  function bindColorPair(colorEl, textEl, cfg = {}) {
    ...
    const start = () => {
      const targets = ensureFillEditSession00997_(`color-control-${String(cfg.reason || 'color')}-focus`);
      try { cfg.onStart?.(targets); } catch (_) {}
      return targets;
    };
    let liveRaf00909 = 0;
  ...
  bindColorPair(grad1Input, grad1Text, { reason: 'fill-gradient-c1', onStart: (targets) => targets.forEach(prepareFillVisualTarget00999), applyLive: (o) => applyGradientStop01001(1, o), applyFinal: (o) => applyGradientStop01001(1, o) });
  bindColorPair(grad2Input, grad2Text, { reason: 'fill-gradient-c2', onStart: (targets) => targets.forEach(prepareFillVisualTarget00999), applyLive: (o) => applyGradientStop01001(2, o), applyFinal: (o) => applyGradientStop01001(2, o) });
  bindColorPair(grad3Input, grad3Text, { reason: 'fill-gradient-c3', onStart: (targets) => targets.forEach(prepareFillVisualTarget00999), applyLive: (o) => applyGradientStop01001(3, o), applyFinal: (o) => applyGradientStop01001(3, o) });
  ...
  bindColorPair(filterColorInput, filterColorText, { reason: 'filter-color', onStart: (targets) => targets.forEach(prepareFilterTarget00999), applyLive: applyFilterSolidColor01001, applyFinal: applyFilterSolidColor01001 });
  bindColorPair(fGrad1Input, fGrad1Text, { reason: 'filter-gradient-c1', onStart: (targets) => targets.forEach(prepareFilterTarget00999), applyLive: (o) => applyFilterGradientStop01001(1, o), applyFinal: (o) => applyFilterGradientStop01001(1, o) });
  bindColorPair(fGrad2Input, fGrad2Text, { reason: 'filter-gradient-c2', onStart: (targets) => targets.forEach(prepareFilterTarget00999), applyLive: (o) => applyFilterGradientStop01001(2, o), applyFinal: (o) => applyFilterGradientStop01001(2, o) });
  bindColorPair(fGrad3Input, fGrad3Text, { reason: 'filter-gradient-c3', onStart: (targets) => targets.forEach(prepareFilterTarget00999), applyLive: (o) => applyFilterGradientStop01001(3, o), applyFinal: (o) => applyFilterGradientStop01001(3, o) });
