...
// 01003: Native color-picker live preview no longer repaints the real Fill/Filter backgrounds.
// Solid Fill/Filter use temporary RGB compositor layers whose only live mutations are opacity.
// The canonical color is written once at final change, then one Store commit is emitted.
...
<div class="design-field">
  ...
  function markFillAuthority00999(target) {
    if (target instanceof HTMLElement) target.style.setProperty(FILL_AUTHORITY_VERSION_VAR_00999, '01003');
  }
...
function getTargets() {
  ...
  function canonicalizeFilterAuthority00999(target, cs = null) {
    ...
    target.style.setProperty(FILL_AUTHORITY_VERSION_VAR_00999, '01003');
  ...
  function bindColorPair(colorEl, textEl, cfg = {}) {
    ...
    colorEl.addEventListener('blur', () => {
      setTimeout(() => {
        ...
        flushLive01002(false);
        if (compositorEntries01003.length) cleanupSolidColorCompositor01003(compositorEntries01003, cfg.compositorKind || '');
        endFillEditSession00779_('color-picker-blur-01003');
        resetLocal01002();
      }, 0);
    });
  }
