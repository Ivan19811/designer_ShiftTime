...
<div class="design-field">
  ...
  function fillGradientImage00999() {
    return `linear-gradient(var(${FILL_GRADIENT_ANGLE_VAR_00999},90deg), var(${FILL_GRADIENT_C1_VAR_00999},#0f172a) var(${FILL_GRADIENT_P1_VAR_01009},0%), var(${FILL_GRADIENT_C2_VAR_00999},#1e293b) var(${FILL_GRADIENT_P2_VAR_01009},50%), var(${FILL_GRADIENT_C3_VAR_00999},#334155) var(${FILL_GRADIENT_P3_VAR_01009},100%))`;
  }
  ...
  function writeGradientState01009(target, state = {}) {
    ...
    const positions = Array.isArray(state.gradientPositions) ? state.gradientPositions : [0,50,100];
  ...
  function markFillAuthority00999(target) {
    if (target instanceof HTMLElement) target.style.setProperty(FILL_AUTHORITY_VERSION_VAR_00999, '01009');
  }
...
function getTargets() {
  ...
  function canonicalizeFilterAuthority00999(target, cs = null) {
    ...
    target.style.setProperty(FILL_AUTHORITY_VERSION_VAR_00999, '01009');
...
// --- УНІФІКОВАНИЙ РЕНДЕР ФОНУ ЧЕРЕЗ ::before (.st-bgfx) ---
  ...
  function prepareFillVisualTarget00999(el) {
    ...
    if (!target.classList.contains('st-bgfx')) {
      ...
      applyFillToElement(target, {
        ...
        gradientAlphas: own.gradientAlphas, gradientPositions: own.gradientPositions,
        ...
        filterMode: own.filterMode, filterColorRaw: own.filterColor,
        filterG1Raw: own.filterGradientColors?.[0], filterG2Raw: own.filterGradientColors?.[1], filterG3Raw: own.filterGradientColors?.[2],
        filterAngle: own.filterAngle, filterOpacity: (own.filterOpacityPct ?? 0) / 100
      });
    }
    markFillAuthority00999(target);
  ...
  function readActiveFillState01007(element) {
    ...
    if (fxBg) {
      if (/linear-gradient\(/i.test(fxBg)) mode = 'gradient';
      else if (/url\(/i.test(fxBg)) mode = 'image';
      else mode = 'color';
    }
    if (!mode) {
      if (/gradient\(/i.test(visualBg)) mode = 'gradient';
      else if (/url\(/i.test(visualBg)) mode = 'image';
      else mode = 'color';
    }
    ...
    const imageSource = fxBg || authoredBgImage || authoredBackground;
    const imageMatch = String(imageSource || '').match(/url\(["']?(.*?)["']?\)/i);
    const imageUrl = imageMatch?.[1] || '';
    const sizeRaw = prop('--st-bgfx-bg-size', false) || ownStyleValue01007(style, 'background-size') || cssField('backgroundSize', 'background-size') || 'cover';
    ...
    const posRaw = prop('--st-bgfx-bg-pos', false) || ownStyleValue01007(style, 'background-position') || cssField('backgroundPosition', 'background-position') || 'center center';
