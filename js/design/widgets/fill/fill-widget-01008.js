...
<div class="design-field">
  ...
  function markFillAuthority00999(target) {
    if (target instanceof HTMLElement) target.style.setProperty(FILL_AUTHORITY_VERSION_VAR_00999, '01008');
  }
...
function getTargets() {
  ...
  function canonicalizeFilterAuthority00999(target, cs = null) {
    ...
    target.style.setProperty(FILL_AUTHORITY_VERSION_VAR_00999, '01008');
...
// --- УНІФІКОВАНИЙ РЕНДЕР ФОНУ ЧЕРЕЗ ::before (.st-bgfx) ---
  ...
  function readActiveFillState01007(element) {
    ...
    const parsedGradientColors = colorTokens01007(visualBg, ['#0f172a','#1e293b','#334155']);
    const gradientColors = gradVars.map((v, i) => v || parsedGradientColors[i] || ['#0f172a','#1e293b','#334155'][i]);
