...
// 00998: Fill/background and authored block-surface are independent visual layers.
// Background opacity controls only the explicit Fill layer; block transparency controls only the original/container surface.
// Neither control rewrites or re-captures the other layer while dragging. Continuous controls stay RAF-only with one final Store commit.
...
function getTargets() {
  ...
  function releaseOwnedFxBackgroundIfUnused00995(target) {
    ...
    if (ownsBg) {
      ...
      for (const prop of ['--st-bgfx-bg','--st-bgfx-bg-color','--st-bgfx-bg-opacity','--st-bgfx-bg-size','--st-bgfx-bg-pos','--st-bgfx-bg-repeat','--st-bgfx-bg-pos-x','--st-bgfx-bg-pos-y','--st-bgfx-gray','--st-bgfx-filter','--st-bgfx-filter-opacity',FILL_LAYER_IMAGE_VAR_00998,FILL_FILTER_IMAGE_VAR_00998]) {
  ...
  function ensureTrueBlockSurfaceLayer00996(target, snap) {
    ...
    target.style.setProperty(BLOCK_SURFACE_LAYER_VERSION_VAR_00998, '00998');
    ...
    if (oldMergedSurface00998) {
      ...
      target.style.setProperty(FILL_LAYER_IMAGE_VAR_00998, 'none');
      target.style.setProperty('--st-bgfx-bg-opacity', '1');
      delete target.dataset.stFillMode;
      delete target.dataset.stFillCustomLayer;
    } else if (explicit00998 || snap?.hadBgfx) {
      const preserved = currentFxBg || fx.bg || 'none';
      target.style.setProperty(FILL_LAYER_IMAGE_VAR_00998, asLayerImage00998(preserved));
      if (snap?.hadBgfx && !explicit00998) target.dataset.stFillCustomLayer = '1';
    } else if (!target.style.getPropertyValue(FILL_LAYER_IMAGE_VAR_00998).trim()) {
      target.style.setProperty(FILL_LAYER_IMAGE_VAR_00998, 'none');
    }
    ...
    if (!target.style.getPropertyValue('--st-bgfx-bg-opacity').trim()) target.style.setProperty('--st-bgfx-bg-opacity', fx.bgOpacity || '1');
    if (!target.style.getPropertyValue('--st-bgfx-bg-size').trim()) target.style.setProperty('--st-bgfx-bg-size', fx.bgSize || 'auto');
    if (!target.style.getPropertyValue('--st-bgfx-bg-pos').trim()) target.style.setProperty('--st-bgfx-bg-pos', fx.bgPos || '0% 0%');
    if (!target.style.getPropertyValue('--st-bgfx-bg-repeat').trim()) target.style.setProperty('--st-bgfx-bg-repeat', fx.bgRepeat || 'no-repeat');
    if (!target.style.getPropertyValue('--st-bgfx-gray').trim()) target.style.setProperty('--st-bgfx-gray', fx.gray || '0');
    if (fx.filter && !target.style.getPropertyValue('--st-bgfx-filter').trim()) target.style.setProperty('--st-bgfx-filter', fx.filter);
    if (fx.filter && !target.style.getPropertyValue(FILL_FILTER_IMAGE_VAR_00998).trim()) target.style.setProperty(FILL_FILTER_IMAGE_VAR_00998, asLayerImage00998(fx.filter));
  ...
  function prepareBlockSurfaceTarget00997(el) {
    ...
    // The authored backdrop declaration is neutralized once. Live ticks only change the variable.
    ...
    // Same for shadow: keep the expensive source once and drive the result through one variable.
  ...
  function applyBlockSurfaceFxToElement00995(el, transparencyPct, blurPct) {
    ...
    // [00997] Hot path: no getComputedStyle, no snapshot encode/decode, no background migration.
  ...
  function applyElementVisualFxToElement00994(el, opacityPct, blurPct) {
    ...
    const surfaceAlpha00996 = Number.parseFloat(target.style.getPropertyValue('--st-block-surface-alpha'));
    const effectiveSurfaceAlpha00996 = Number.isFinite(surfaceAlpha00996) ? Math.max(0, Math.min(1, surfaceAlpha00996)) : 1;
    target.style.setProperty('box-shadow', scaleShadowOpacity00994(sourceShadow, opacity * effectiveSurfaceAlpha00996), 'important');
    return true;
  }
...
// --- УНІФІКОВАНИЙ РЕНДЕР ФОНУ ЧЕРЕЗ ::before (.st-bgfx) ---
    ...
    if (needsFx) {
      ...
      target.style.setProperty('--st-bgfx-bg', bgVal);
      target.style.setProperty(FILL_LAYER_IMAGE_VAR_00998, asLayerImage00998(bgVal));
      target.style.setProperty('--st-bgfx-bg-opacity', String(alpha));
      target.style.setProperty('--st-bgfx-gray', String(gray));
      ...
      if (fMode === 'off' || fOp <= 0) {
        ...
        target.style.removeProperty('--st-bgfx-filter');
        target.style.setProperty(FILL_FILTER_IMAGE_VAR_00998, 'none');
        ...
        const filterColor00998 = hexToRgba(fc, fOp);
        target.style.setProperty('--st-bgfx-filter', filterColor00998);
        target.style.setProperty(FILL_FILTER_IMAGE_VAR_00998, `linear-gradient(${filterColor00998}, ${filterColor00998})`);
        ...
        const filterGradient00998 = `linear-gradient(${fa}deg, ${c1}, ${c2}, ${c3})`;
        target.style.setProperty('--st-bgfx-filter', filterGradient00998);
        target.style.setProperty(FILL_FILTER_IMAGE_VAR_00998, filterGradient00998);
        target.style.setProperty('--st-bgfx-filter-opacity', '1');
      } else {
        target.style.setProperty('--st-bgfx-filter-opacity', '0');
        target.style.removeProperty('--st-bgfx-filter');
  ...
  function applyBackgroundOpacity00998(opts = {}) {
    ...
    for (const el of targets) {
      ...
      if (target.classList.contains('st-bgfx')) {
        ...
        const state = getFillStateFromUI();
        applyFillToElement(target, state);
      }
    }
  ...
  function bindColorPair(colorEl, textEl) {
    ...
    const start = () => ensureFillEditSession00997_('color-control-focus');
    let liveRaf00909 = 0;
  ...
  // кут градієнта 0..360
  bindFillRangeLive00997(gradAngleRange, gradAngleNumber, { min: 0, max: 360, fallback: 90, reason: 'gradient-angle' });

  // --- Ч/Б фону (0..100) ---
  bindFillRangeLive00997(grayRange, grayNumber, { min: 0, max: 100, fallback: 0, reason: 'background-gray' });
  ...
  if (imagePosXRange) {
    bindFillRangeLive00997(imagePosXRange, null, { min: 1, max: 100, fallback: 50, reason: 'image-pos-x', onValue: () => syncPosLabel(imagePosXRange, imagePosXVal) });
    syncPosLabel(imagePosXRange, imagePosXVal);
  }
  if (imagePosYRange) {
    bindFillRangeLive00997(imagePosYRange, null, { min: 1, max: 100, fallback: 50, reason: 'image-pos-y', onValue: () => syncPosLabel(imagePosYRange, imagePosYVal) });
  ...
  if (imageScaleRange) {
    bindFillRangeLive00997(imageScaleRange, null, { min: 0, max: 200, fallback: 100, reason: 'image-scale', onValue: () => syncPctLabel(imageScaleRange, imageScaleVal) });
    syncPctLabel(imageScaleRange, imageScaleVal);
  }
  if (imageScaleXRange) {
    bindFillRangeLive00997(imageScaleXRange, null, { min: 0, max: 200, fallback: 100, reason: 'image-scale-x', onValue: () => syncPctLabel(imageScaleXRange, imageScaleXVal) });
    syncPctLabel(imageScaleXRange, imageScaleXVal);
  }
  if (imageScaleYRange) {
    bindFillRangeLive00997(imageScaleYRange, null, { min: 0, max: 200, fallback: 100, reason: 'image-scale-y', onValue: () => syncPctLabel(imageScaleYRange, imageScaleYVal) });
  ...
  // прозорість фільтра
  bindFillRangeLive00997(filterOpacityRange, filterOpacityNumber, { min: 0, max: 100, fallback: 0, reason: 'filter-opacity' });
