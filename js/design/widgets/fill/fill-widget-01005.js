...
<div class="design-field">
  ...
  function markFillAuthority00999(target) {
    if (target instanceof HTMLElement) target.style.setProperty(FILL_AUTHORITY_VERSION_VAR_00999, '01005');
  }
...
function getTargets() {
  ...
  function canonicalizeFilterAuthority00999(target, cs = null) {
    ...
    target.style.setProperty(FILL_AUTHORITY_VERSION_VAR_00999, '01005');
...
// --- УНІФІКОВАНИЙ РЕНДЕР ФОНУ ЧЕРЕЗ ::before (.st-bgfx) ---
  ...
  function ensureCustomPicker01005() {
    ...
    root.dataset.stFillColorPicker01005 = '1';
    root.hidden = true;
    root.innerHTML = `
    ...
    const close = (mode = 'commit') => {
      ...
      api.dragRect = null;
      if (mode === 'cancel') {
        api.hex = api.startHex;
        syncColorTrigger01005(trigger, api.startHex);
        trigger.dispatchEvent(new CustomEvent(CUSTOM_COLOR_LIVE_EVENT_01005, { detail: { hex: api.startHex, source: 'custom-picker-cancel-01005' } }));
        trigger.dispatchEvent(new CustomEvent(CUSTOM_COLOR_CANCEL_EVENT_01005, { detail: { hex: api.startHex } }));
      } else {
        emitLive(true);
        syncColorTrigger01005(trigger, api.hex);
  ...
  function bindColorPair(colorEl, textEl, cfg = {}) {
    ...
    colorEl.addEventListener('blur', () => {
      setTimeout(() => {
        ...
        flushLive01002(false);
        if (compositorEntries01003.length) cleanupSolidColorCompositor01003(compositorEntries01003, cfg.compositorKind || '');
        endFillEditSession00779_('color-picker-blur-01005');
        resetLocal01002();
      }, 0);
    });
  }
