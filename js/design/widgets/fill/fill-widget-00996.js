...
// 00996: Fill owns a true block surface layer. Authored/template background and backdrop blur are moved under widget authority while block-surface FX is active.
// 100% block transparency leaves content + outer border visible; block blur affects only the backdrop behind the block.
      ...
      <div class="design-field">
        ...
        <div class="design-field__row design-fill-extra">
          ...
          <div class="design-subnote design-fill-block-surface-fx-note">
            Прозорість блока не змінює текст, кнопки та рамку. Розмитість блока розмиває тільки банер/фон позаду контейнера.
          </div>
...
function getTargets() {
  ...
  function ensureTrueBlockSurfaceLayer00996(target, snap) {
    ...
    const sourceBg = currentFxBg && currentFxBg !== 'none'
      ? currentFxBg
      : String(fx.bg && fx.bg !== 'none' ? fx.bg : (snap?.computed?.backgroundImage || 'none'));
    const currentFxColor = (target.style.getPropertyValue('--st-bgfx-bg-color') || cs.getPropertyValue('--st-bgfx-bg-color') || '').trim();
    const sourceColor = currentFxColor || fx.bgColor || snap?.computed?.backgroundColor || 'transparent';

    if (!target.classList.contains('st-bgfx')) {
      target.classList.add('st-bgfx');
      target.style.setProperty(ELEMENT_FX_OWNS_BG_VAR_00994, '1');
    } else if (!target.style.getPropertyValue(ELEMENT_FX_OWNS_BG_VAR_00994).trim()) {
      target.style.setProperty(ELEMENT_FX_OWNS_BG_VAR_00994, '0');
    }

    target.style.setProperty('--st-bgfx-bg', sourceBg || 'none');
    target.style.setProperty('--st-bgfx-bg-color', sourceColor || 'transparent');
    if (!target.style.getPropertyValue('--st-bgfx-bg-opacity').trim()) target.style.setProperty('--st-bgfx-bg-opacity', fx.bgOpacity || '1');
    if (!target.style.getPropertyValue('--st-bgfx-bg-size').trim()) target.style.setProperty('--st-bgfx-bg-size', fx.bgSize || snap?.computed?.backgroundSize || 'auto');
    if (!target.style.getPropertyValue('--st-bgfx-bg-pos').trim()) target.style.setProperty('--st-bgfx-bg-pos', fx.bgPos || snap?.computed?.backgroundPosition || '0% 0%');
    if (!target.style.getPropertyValue('--st-bgfx-bg-repeat').trim()) target.style.setProperty('--st-bgfx-bg-repeat', fx.bgRepeat || snap?.computed?.backgroundRepeat || 'repeat');
    ...
    // 00996: inline !important is the surface authority. It defeats authored template
    // background declarations such as .shifttime02-hero-copy{background:...!important}
    // without adding any template-specific runtime special case.
    target.style.setProperty('background', 'transparent', 'important');
    target.style.setProperty('background-image', 'none', 'important');
    target.style.setProperty('background-color', 'transparent', 'important');
  }
  ...
  function clearBlockSurfaceFx00995(target) {
    ...
    if (snap) {
      restoreBlockSurfaceInline00996(target, snap);
  ...
  function applyBlockSurfaceFxToElement00995(el, transparencyPct, blurPct) {
    ...
    // The widget owns backdrop blur while the block-surface layer is active, including 0%.
    target.style.setProperty('-webkit-backdrop-filter', `blur(${blurPx}px)`, 'important');
    target.style.setProperty('backdrop-filter', `blur(${blurPx}px)`, 'important');
    const elementOpacity = effectiveElementOpacity00996(target);
    target.style.setProperty('box-shadow', scaleShadowOpacity00994(snap?.computed?.boxShadow || 'none', surfaceAlpha * elementOpacity), 'important');
    return true;
  }
