...
// 00999: Fill controls use independent visual layers and one canonical authority per continuous effect.
...
<div class="design-field">
  ...
  function markFillAuthority00999(target) {
    if (target instanceof HTMLElement) target.style.setProperty(FILL_AUTHORITY_VERSION_VAR_00999, '00999');
  }
...
function getTargets() {
  ...
  function parseFilterAuthority00999(target, cs = null) {
    ...
    if (/var\(--st-bgfx-filter-c1/i.test(source) && c1Var) {
      ...
      const same = colors.every((hex) => hex === colors[0]);
      return { mode: same ? 'color' : 'gradient', opacity, image: source, colors, angle, legacyEmbeddedAlpha: false };
    }
  ...
  function filterImageFromCanonical00999(mode = 'off') {
    if (mode === 'color') {
      return `linear-gradient(var(${FILTER_C1_VAR_00999},#000000), var(${FILTER_C1_VAR_00999},#000000))`;
    }
    if (mode === 'gradient') {
  ...
  function canonicalizeFilterAuthority00999(target, cs = null) {
    ...
    target.style.setProperty(FILL_AUTHORITY_VERSION_VAR_00999, '00999');
...
// --- УНІФІКОВАНИЙ РЕНДЕР ФОНУ ЧЕРЕЗ ::before (.st-bgfx) ---
  ...
  function resetFillStyles_() {
    ...
    targets.forEach(el => {
      ...
      for (const prop of [FILL_GRADIENT_ANGLE_VAR_00999,FILL_GRADIENT_C1_VAR_00999,FILL_GRADIENT_C2_VAR_00999,FILL_GRADIENT_C3_VAR_00999,FILTER_ANGLE_VAR_00999,FILTER_C1_VAR_00999,FILTER_C2_VAR_00999,FILTER_C3_VAR_00999,IMAGE_SCALE_VAR_00999,IMAGE_SCALE_X_VAR_00999,IMAGE_SCALE_Y_VAR_00999]) t.style.removeProperty(prop);
