...
// 01002: Native color-picker hot paths are isolated to one canonical style write per animation frame.
...
<div class="design-field">
  ...
  function markFillAuthority00999(target) {
    if (target instanceof HTMLElement) target.style.setProperty(FILL_AUTHORITY_VERSION_VAR_00999, '01002');
  }
...
function getTargets() {
  ...
  function canonicalizeFilterAuthority00999(target, cs = null) {
    ...
    target.style.setProperty(FILL_AUTHORITY_VERSION_VAR_00999, '01002');
...
// --- УНІФІКОВАНИЙ РЕНДЕР ФОНУ ЧЕРЕЗ ::before (.st-bgfx) ---
  ...
  function bindColorPair(colorEl, textEl, cfg = {}) {
    ...
    let liveRaf01002 = 0;
    let latestHex01002 = normalizeHex(colorEl.value || textEl.value || '#000000');
    ...
    const flushLive01002 = (applyPending = false) => {
      if (liveRaf01002) {
        ...
        if (applyPending) {
          ...
          if (directLive01002 && typeof cfg.applyLiveDirect === 'function') cfg.applyLiveDirect(liveTargets01002, latestHex01002);
        }
      }
    };

    const scheduleLive01002 = () => {
      latestHex01002 = normalizeHex(colorEl.value);
      if (!locallyStarted01002) start();
      if (liveRaf01002) return;
      liveRaf01002 = requestAnimationFrame(() => {
        liveRaf01002 = 0;
        // Keep the inspector text field coalesced too; never rewrite it for every raw
        // native color-picker event.
        textEl.value = latestHex01002;
        if (directLive01002 && typeof cfg.applyLiveDirect === 'function') {
    ...
    textEl.addEventListener('change', () => {
      ...
      start();
      latestHex01002 = normalizeHex(textEl.value);
      textEl.value = latestHex01002;
      colorEl.value = latestHex01002;
      if (typeof cfg.applyFinal === 'function') cfg.applyFinal({ live: false, useSessionTargets: true, endSession: true });
      else applyFill({ live: false, useSessionTargets: true, endSession: true });
  ...
  bindColorPair(filterColorInput, filterColorText, { reason: 'filter-color', prepareTargets: prepareFilterSolidColorSession01002, applyLiveDirect: liveFilterSolidColor01002, applyLive: applyFilterSolidColor01001, applyFinal: applyFilterSolidColor01001 });
