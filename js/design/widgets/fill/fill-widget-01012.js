  ...
  sectionEl.innerHTML = `
    ...
    <div class="design-section__body">
      ...
      <div class="design-field" data-fill-group="image" hidden>
        ...
        <!-- 01030: preview has the same aspect as the selected block/banner. -->
        <button
          type="button"
          data-fill="imagePreview"
          title="Відкрити галерею та змінити фонову картинку"
          style="
          ...
          "
          aria-label="Превʼю фонової картинки"
          hidden
        ></button>
...
// --- УНІФІКОВАНИЙ РЕНДЕР ФОНУ ЧЕРЕЗ ::before (.st-bgfx) ---
  ...
  function updateImagePickButtonPreview(url, visual = {}) {
    ...
    imagePreviewBtn01030.style.backgroundImage = cleanUrl ? `url("${cleanUrl.replace(/([\\"])/g, '\\$1')}")` : 'none';
    ...
    imagePreviewBtn01030.hidden = !cleanUrl;
    imagePreviewBtn01030.title = cleanUrl ? 'Фон вибраного елемента · клік — відкрити галерею' : 'Фонової картинки немає';
  }
  ...
  if (imageUrlInput) {
    ...
    imageUrlInput.addEventListener('input', () => {
      updateImagePickButtonPreview(imageUrlInput.value || '', { aspect: parseFloat(imagePreviewBtn01030?.style.aspectRatio) || (16/9), position: imagePosSelect?.value || 'center center', size: imageSizeSelect?.value || 'cover' });
  ...
  function readActiveFillState01007(element) {
    ...
    const hasRealImageLayer01030 = !!(activeSliderImage01030?.src || literalImageUrl01030);
    ...
    const imageUrl = String(activeSliderImage01030?.src || literalImageUrl01030 || '').trim();
    const sizeRaw = activeSliderImage01030?.fit || prop('--st-bgfx-bg-size', false) || ownStyleValue01007(style, 'background-size') || cssField('backgroundSize', 'background-size') || 'cover';
    ...
    const posRaw = activeSliderImage01030?.position || prop('--st-bgfx-bg-pos', false) || ownStyleValue01007(style, 'background-position') || cssField('backgroundPosition', 'background-position') || 'center center';
