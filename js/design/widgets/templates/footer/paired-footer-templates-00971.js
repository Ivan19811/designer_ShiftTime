// 00971-FOOTER-CONTACT-LEGAL-IN-CARD
// Paired 01–55 keep the same visual family/structure; 00969 only hardens authored contrast and contact flow.
...
const AUTHORED_VERSION_00966 = '00971';
...
function logoBlock00966_(pair,id,recipe,theme){
  ...
  return node00966_(pair,'block','div',id,{class:'hb-elem st-block st-block--text st-block--logo hf00966-logo','data-block-kind':'text','data-block-role':'logo','data-name':'Лого','data-hb-tip':'Лого','data-logo-mode':'logo-text-subtitle','data-logo-source':'icon','data-logo-pos':'left','data-logo-click-area':'all','data-logo-fit':'contain','data-logo-title-size':String(theme.typography.logoTitleSize).replace('px',''),'data-logo-subtitle-size':String(theme.typography.logoSubtitleSize).replace('px','')},
    {width:'100%','min-width':'0',display:'flex','align-items':'center',gap:'12px',background:'transparent',border:'0',overflow:'visible',padding:'0',color:'inherit','box-sizing':'border-box'},kids);
}
...
function menu00966_(pair,id,label,items,theme){
  ...
  const menuText=theme.menu.text;
  const menuHover=theme.menu.hoverText||menuText;
  const menuActive=theme.menu.activeText||menuText;
  const menuFs=theme.menu.fontSize||'14px';
  const menuFw=theme.menu.fontWeight||'700';
  return node00966_(pair,'block','div',id,{class:'hb-elem st-block st-block--menu hf00966-menu','data-block-kind':'menu','data-name':label,'data-hb-tip':label,'data-st-menu':'1','data-menu-variant':'footer','data-menu-level1-direction':'column','data-menu-items':JSON.stringify(list.map(([text,href])=>({text,href:href||'#',children:[]})))},
    {width:'100%','min-width':'0','max-width':'100%',display:'flex','align-items':'flex-start',background:'transparent',border:'0',overflow:'visible',padding:'0',color:menuText,'box-sizing':'border-box',
      '--st-menu-link-color':menuText,'--st-menu-link-color-h':menuHover,'--st-menu-link-fs':menuFs,'--st-menu-link-fw':menuFw,
      '--st-menu-l1-color':menuText,'--st-menu-l1-h-color':menuHover,'--st-menu-l1-o-color':menuActive,'--st-menu-l1-c-color':menuActive,
      '--st-menu-l1-fs':menuFs,'--st-menu-l1-h-fs':menuFs,'--st-menu-l1-o-fs':menuFs,'--st-menu-l1-c-fs':menuFs,
      '--st-menu-l1-fw':menuFw,'--st-menu-l1-h-fw':menuFw,'--st-menu-l1-o-fw':menuFw,'--st-menu-l1-c-fw':menuFw},[
...
function infoLink00966_(pair,id,label,href,theme,iconKind,{compact=false}={}){
  ...
  return node00966_(pair,'block','div',id,{class:'hb-elem st-block st-block--text st-block--link hf00966-info-link','data-block-kind':'text','data-block-role':'link','data-name':label,'data-hb-tip':label},
    {width:blockWidth,'min-width':'0','max-width':'100%','min-height':compact?'30px':'38px',display:'flex','align-items':'center',background:'transparent',border:'0',overflow:'visible',padding:'0',color:theme.links.color,'box-sizing':'border-box',flex:blockFlex},[
      node00966_(pair,'element','a','',{href,class:'st-text-edit hf00966-info-link__a','data-st-text-target':'1',contenteditable:'true',draggable:'true',spellcheck:'false'},
        {display:'flex','flex-direction':'row','flex-wrap':'nowrap','align-items':'center','justify-content':'flex-start',gap:compact?'7px':'9px',width:linkWidth,'min-width':'0','max-width':'100%','min-height':compact?'30px':'38px',height:'auto',flex:'0 1 auto',padding:'0',border:'0','border-radius':'0',background:'transparent','box-shadow':'none',color:'inherit','font-size':theme.links.fontSize,'font-weight':theme.links.fontWeight,'line-height':theme.links.lineHeight,'text-decoration':'none','white-space':'normal','word-break':'normal','overflow-wrap':'anywhere','box-sizing':'border-box'},[
          node00966_(pair,'element','span','',{class:'hf00966-mini-icon','aria-hidden':'true'},{width:compact?'24px':'30px',height:compact?'24px':'30px',display:'inline-grid','place-items':'center','border-radius':theme.radius.md,background:theme.icons.bg,color:theme.icons.color,border:`${theme.icons.borderWidth} solid ${theme.icons.borderColor}`,flex:'0 0 auto'},[svg]),
          text00966_(label)
        ])
    ]);
}
...
function buildModel00966_(pair){
  const t=pair.theme, recipe=getHeaderFooterVisualRecipeByNo00969(pair.no);
  ...
  const ctaLevel=node00966_(pair,'level','div',`hf${pair.no}_footer_level_001`,{class:'st-row hf00966-cta-row','data-st-node':'level','data-layout-mode':'fr','data-layout-orient':'row'},
    {display:'grid','grid-template-columns':'minmax(0,1.35fr) minmax(280px,.65fr)','align-items':'center',gap:'28px',width:'min(1320px,calc(100% - 48px))',margin:'0 auto',padding:'30px 34px',background:ctaPanelBackground,color:ctaPanelColor,border:ctaPanelBorder,'border-radius':panelRadius,'box-shadow':ctaPanelShadow,'backdrop-filter':ctaPanelBackdrop,overflow:'visible','box-sizing':'border-box',position:'relative'},[
      container00966_(pair,`hf${pair.no}_footer_container_001`,'CTA текст',[
        blockText00966_(pair,`hf${pair.no}_footer_block_001`,`ПАРА ${pair.no} · ${pair.shortName}`,'eyebrow',{color:'inherit',opacity:'.95','font-size':'11px','font-weight':'900','line-height':'1.2','letter-spacing':'.09em','text-transform':'uppercase'}),
        ...
        blockText00966_(pair,`hf${pair.no}_footer_block_003`,modernDescription00966_(pair),'text',{color:'inherit',opacity:'.95','font-size':'14px','font-weight':'650','line-height':'1.55','max-width':'760px'})
      ],{gap:'10px'}),
  ...
  const brandCard=container00966_(pair,`hf${pair.no}_footer_container_003`,'Бренд',[
    ...
    blockText00966_(pair,`hf${pair.no}_footer_block_007`,modernDescription00966_(pair),'text',{color:panelColor,opacity:'.76','font-size':'13px','font-weight':'650','line-height':'1.55'}),
  ...
  const navCard=container00966_(pair,`hf${pair.no}_footer_container_005`,'Навігація',[
    blockText00966_(pair,`hf${pair.no}_footer_block_008`,'Навігація','heading',{color:panelColor,'font-size':'13px','font-weight':'900','line-height':'1.2','letter-spacing':'.07em','text-transform':'uppercase'},'st-block--heading'),
  ...
  const infoCard=container00966_(pair,`hf${pair.no}_footer_container_006`,'Інформація',[
    blockText00966_(pair,`hf${pair.no}_footer_block_010`,'Інформація','heading',{color:panelColor,'font-size':'13px','font-weight':'900','line-height':'1.2','letter-spacing':'.07em','text-transform':'uppercase'},'st-block--heading'),
  ...
  const contactCard=container00966_(pair,`hf${pair.no}_footer_container_007`,'Контакти',[
    blockText00966_(pair,`hf${pair.no}_footer_block_012`,'Контакти','heading',{color:panelColor,'font-size':'13px','font-weight':'900','line-height':'1.2','letter-spacing':'.07em','text-transform':'uppercase'},'st-block--heading'),
    ...
    node00966_(pair,'container','div',`hf${pair.no}_footer_container_009`,{class:'st-block hf00966-legal hf00966-contact-legal','data-st-node':'container','data-layout-mode':'flex','data-layout-orient':'column','data-name':'Юридичні посилання'},
      {width:'100%','min-width':'0','max-width':'100%',display:'flex','flex-direction':'column','align-items':'stretch','justify-content':'flex-start',gap:'4px',background:'transparent',border:'0','border-top':`1px solid ${t.colors.border}`,overflow:'visible',padding:'8px 0 0','box-sizing':'border-box'},[
        infoLink00966_(pair,`hf${pair.no}_footer_block_018`,'Конфіденційність','#privacy',t,'shield',{compact:true}),
        infoLink00966_(pair,`hf${pair.no}_footer_block_019`,'Умови','#terms',t,'shield',{compact:true})
      ])
  ],{padding:'18px',background:panelBackgroundAlt,color:panelColor,border:panelBorder,'border-radius':panelRadius,gap:'8px','min-height':'0',height:'auto',overflow:'visible'});
  ...
  return {version:MODEL_VERSION_00966,schema:'section-level-container-block-dom-v1',scope:'footer',templateId:pair.footerTemplateId,sourcePolicy:'PAIR_HEADER_VISUAL_RECIPE_IS_SOURCE_OF_TRUTH_00971',renderPolicy:'DOM is rendered from this canonical model; no runtime normalizer/adapter.',root};
}
...
const RAW_00966=getHeaderFooterStylePairs00965().map((pair)=>{
  ...
  return {
    ...
    meta:{source:'system',palette:pair.palette,pairId:pair.pairId,pairNo:pair.no,pairName:pair.name,pairedHeaderTemplateId:pair.headerTemplateId,pairContract:'header-footer-style-pair-v2-00971',visualRecipeContract:'header-footer-full-visual-recipe-v1-00969',modelContract:MODEL_VERSION_00966,singleSourceOfTruth:'model',standardKeys00965:true,authoredFooter00971:true,headerVisualParity00971:true,headerIconCount00971:recipe?.headerIconCount||0,headerBackgroundCarried00971:!!recipe?.rootBackground,headerSystemBackgroundCarried00971:!!recipe?.hasSystemBackground,tools:['section','row','container','logo','menu','text','button','link','icon']},
...
export const PAIRED_FOOTER_TEMPLATES_00971=Object.freeze(RAW_00966.map(Object.freeze));
export function getPairedFooterTemplates00971(){ return PAIRED_FOOTER_TEMPLATES_00971.map(clone00966_); }
