...
function persist(reason = 'special-effects') {
  // 01019: Main is JSON-primary. The slider editor writes its draft/final config to
  // data-st-fx-bg-slider on the live DOM, but ST_SAVE_ROOT_DOM_HTML rebuilds Main from
  // SiteFrameStore before snapshotting. Commit the authored slider/bind datasets into
  // the canonical Store FIRST, otherwise the rebuild restores the previous bg.fit /
  // scale / scaleX / scaleY / position values and the user sees the image jump back.
  let mainDatasetCommit = null;
  try {
    mainDatasetCommit = window.ST_SITE_FRAME_STORE_AUTHORITY_00876?.commitMainSpecialEffectsDataset01019?.(reason) || null;
  } catch (error) {
    try {
      window.__ST_ALL_LOG__?.push?.('special-effects:main-dataset-commit-error-01019', {
        version: '01019-main-special-effects-json-persistence-parity',
        reason: String(reason || ''),
  ...
  // SiteFrame root save renders Main from canonical JSON synchronously. Runtime
  ...
  rehydrateBackgroundFx00992(reason);
  try {
    window.__ST_ALL_LOG__?.push?.('special-effects:apply-persisted-01019', {
      version: '01019-main-special-effects-json-persistence-parity',
      reason: String(reason || ''),
      mainDatasetCommit,
      exactStoreBeforeRootRender: mainDatasetCommit?.ok !== false,
    }, mainDatasetCommit?.ok === false ? 'warn' : 'info');
  } catch (_) {}
