...
// 00158 + 00991 — Акордеон "Спецефекти".
// 00991: bound-slide visibility is runtime host state, never canonical child display style.
// Widget slide selection drives the real Hero slide immediately. Selection alone never freezes autoplay;
// only an active resize/drag gesture, actual text focus, or authored hover-pause can pause it.
...
const sliderRuntimeControllers00991 = new WeakMap();
...
function stopFx(el) {
  ...
  timers.delete(el);
  clearSliderResume00990(el);
  sliderRuntimeControllers00991.delete(el);
}
...
function initBackgroundFx(el) {
  ...
  if (getActiveSlideIndexes(cfg).length > 1) {
    ...
    const isAuthoringFrozen00990 = () => {
      ...
      // 00991: a selected Hero/text block is not an authoring lock. This was the
      // reason autoplay stayed stopped forever after a normal edit/selection.
      return paused || editGestureActive || textFocusInsideThisSlider;
    };
    const tick = () => {
      timers.delete(el);
      const c = readConfig(el);
      if (!c.enabled || !c.slides.length) return stopFx(el);
      // 00991: no authoring poll. Active resize/drag/text-focus pauses resume only
      // from explicit lifecycle/focus/hover events; selection itself is never a freeze.
      if (isAuthoringFrozen00990()) {
        armSliderResume00990(
          el,
          () => !isAuthoringFrozen00990(),
          () => {
            const resumed = readConfig(el);
            if (resumed.enabled && resumed.slides.length) schedule(getSlideInterval(resumed, resumed.current || 0));
          }
        );
        return;
      }
    ...
    if (cfg.pauseOnHover) {
      ...
      el.addEventListener('mouseleave', () => {
        paused = false;
        if (!timers.has(el) && !sliderResumeCleanups00990.has(el)) {
          const c = readConfig(el);
          if (c.enabled && c.slides.length) schedule(getSlideInterval(c, c.current || 0));
        }
      }, { passive: true });
    }
    sliderRuntimeControllers00991.set(el, Object.freeze({
      restart() {
...
function getDirectSelectedTarget00990(event, getSelection) {
  ...
  return direct || collectTargets(getSelection)[0] || null;
}
...
function activateWidgetSlide00991(section, getSelection, index, options = {}) {
  ...
  for (const host of targets) {
    ...
    sliderRuntimeControllers00991.get(host)?.restart?.();
  }
...
export function initSpecialEffectsWidget(host, getSelection) {
  ...
  section.addEventListener('click', async (ev) => {
    ...
    if (focusAnimationBtn) {
      ...
      if (Number.isFinite(index) && index >= 0) activateWidgetSlide00991(section, getSelection, index);
    ...
    if (selectSlideBtn) {
      ...
      activateWidgetSlide00991(section, getSelection, index);
