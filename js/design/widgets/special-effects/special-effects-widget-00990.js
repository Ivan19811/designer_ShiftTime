...
// 00158 + 00990 — Акордеон "Спецефекти".
// 00990: bound-slide visibility is runtime host state, never canonical child display style.
// Selecting a slider (or a child inside it) opens this widget directly, and authoring pauses by explicit lifecycle events.
...
function initBackgroundFx(el) {
  ...
  if (getActiveSlideIndexes(cfg).length > 1) {
    ...
    const isAuthoringFrozen00990 = () => {
      ...
      const selectionInsideThisSlider = el.matches('.sf-edit-selected,.sf-selection-current')
      ...
      return paused || editGestureActive || selectionInsideThisSlider || textFocusInsideThisSlider;
    };
    const tick = () => {
      timers.delete(el);
      const c = readConfig(el);
      if (!c.enabled || !c.slides.length) return stopFx(el);
      // 00990: no 250 ms authoring poll. A resize/drag/selection/focus freeze resumes
      // only from explicit lifecycle/selection/focus events.
