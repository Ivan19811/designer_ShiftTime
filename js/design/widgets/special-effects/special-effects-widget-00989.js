...
// 00158 + 00989 — Акордеон "Спецефекти".
// 00989: applied templates rehydrate the existing slider through st:templates-applied; selected/focused bound-slide authoring freezes automatic switching until the edit target is released.
...
function normalizeSlide(slide = {}) {
  ...
  const out = {
    ...
    paused: slide.paused === true || slide.paused === 'true' || slide.enabled === false,
    bg,
  };
...
function readConfig(el) {
  const cfg = Object.assign(defaultFxConfig(), safeJsonParse(el?.getAttribute?.('data-st-fx-bg-slider'), {}) || {});
...
function writeConfig(el, cfg) {
  if (!el) return;
  const clean = Object.assign(defaultFxConfig(), cfg || {});
...
function getActiveSlideIndexes(cfg) {
  ...
  return slides
    .map((slide, index) => (slide && slide.src && !slide.paused ? index : -1))
...
function updateBoundChildren(el, index) {
  try {
    el.querySelectorAll('[data-st-fx-bind-slide]').forEach(child => {
      if (child.closest?.(`[${RUNTIME_ATTR}]`)) return;
      const n = Number(child.getAttribute('data-st-fx-bind-slide'));
      const visible = Number.isFinite(n) && n === index + 1;
      // 00988: slide binding owns visibility only; it never writes geometry. Authored templates often use
      // display:flex/grid !important, so a normal inline display:none loses.
      // Use important only while hidden and remove the property when visible.
...
function initBackgroundFx(el) {
  ...
  if (getActiveSlideIndexes(cfg).length > 1) {
    ...
    const tick = () => {
      ...
      const selectionInsideThisSlider00989 = el.matches('.sf-edit-selected,.sf-selection-current')
        || !!el.querySelector('.sf-edit-selected,.sf-selection-current');
      const activeElement00989 = document.activeElement;
      const textFocusInsideThisSlider00989 = activeElement00989 instanceof HTMLElement
        && el.contains(activeElement00989)
        && !!activeElement00989.closest?.('[contenteditable="true"],[contenteditable="plaintext-only"],.st-text-edit,[data-st-text-target="1"]');
      // 00989: authoring must be spatially stable. The automatic slider keeps running
      // in normal canvas/preview state, but a selected/focused Hero child is frozen
      // until the user leaves it. This prevents the resize owner from becoming
      // display:none between selection, pointerdown and pointerup.
      if (paused || selectionInsideThisSlider00989 || textFocusInsideThisSlider00989) { schedule(250); return; }
      ...
      // 00987: UI contract says "секунд між переходами". The next transition
      // starts after the authored interval itself; animation duration must not be
      // added a second time or a 3 s slider becomes 4–8 s depending on animation.
    ...
    schedule(getSlideInterval(cfg, cfg.current || 0));
  }
}
...
function findSliderHost(el) {
  try { return el?.closest?.('[data-st-fx-bg-slider]') || null; } catch (_) { return null; }
}
...
function getUiDraftSlides(section) {
  ...
  section?.querySelectorAll?.('[data-slide-src]')?.forEach(srcInput => {
    ...
    const slide = {
      ...
      alt: '',
      paused: card?.dataset?.slidePaused === '1',
...
function slideCompareObject(slide = {}) {
  ...
  const paused = slide.paused === true || slide.paused === 'true' || slide.enabled === false;
  const out = { src, link, paused };
...
function renderSlides(listEl, cfg, selectedIndexes = [], activeIndex = -1) {
  ...
  listEl.innerHTML = slides.map((sl, i) => {
    ...
    return `
    <div class="stfx-slide${checked ? ' is-selected' : ''}${active ? ' is-active' : ''}" data-slide-index="${i}" data-slide-paused="${paused ? '1' : '0'}" data-slide-interval="${Number(sl.interval) || ''}" data-slide-duration="${Number(sl.duration) || ''}" data-slide-animation="${esc(sl.animation || '')}" data-slide-overlay="${sl.overlay !== undefined && sl.overlay !== null ? Number(sl.overlay) : ''}">
      ...
      <div class="stfx-slide-actions">
        ...
        <button class="stfx-btn stfx-btn--icon ${paused ? 'stfx-btn--pause' : 'stfx-btn--play'}" type="button" data-toggle-slide-paused="${i}" title="${paused ? 'Слайд на паузі — натисни, щоб увімкнути' : 'Слайд активний — натисни, щоб поставити на паузу'}">${paused ? 'Ⅱ' : '▶'}</button>
...
function previewOverlay(section, getSelection, value) {
  ...
  collectTargets(getSelection).forEach(el => {
...
function syncUi(section, getSelection) {
  ...
  const first = targets[0] || null;
  const cfg = first ? readConfig(first) : defaultFxConfig();
  const boundChild = first && !first.matches?.('[data-st-fx-bg-slider]') ? first : null;
  const host = boundChild ? findSliderHost(boundChild.parentElement) : null;

  section.querySelector('[data-stfx-info]').textContent = first
    ? `Активно: ${targets.length} елемент(и). ${first.classList.contains('st-section') ? 'Секція' : first.classList.contains('st-row') ? 'Рівень' : 'Блок'}`
  ...
  if (bindBox) {
    ...
    const currentBind = Number(first?.getAttribute?.('data-st-fx-bind-slide') || '1') || 1;
...
export function initSpecialEffectsWidget(host, getSelection) {
  ...
  section.addEventListener('click', async (ev) => {
    ...
    if (focusAnimationBtn) {
      ...
      if (Number.isFinite(index) && index >= 0) setActiveSlide(section, index);
      const cfg = first ? readConfig(first) : defaultFxConfig();
    ...
    if (slideCard && !ev.target.closest('button,input,select,textarea')) {
      setActiveSlide(section, Number(slideCard.dataset.slideIndex));
      return;
    }
    ...
    if (pauseBtn) {
      ev.preventDefault();
      const i = Number(pauseBtn.dataset.toggleSlidePaused);
      const cfg = first ? readConfig(first) : defaultFxConfig();
      cfg.slides = getUiDraftSlides(section);
      if (cfg.slides[i]) cfg.slides[i].paused = !cfg.slides[i].paused;
      if (!cfg.slides.length) cfg.slides.push({ src: '', link: '', alt: '', paused: false });
      renderSlides(section.querySelector('[data-slide-list]'), cfg, getSelectedSlideIndexes(section), i);
      markSlidesDirtySoon(section, getSelection);
      if (targets.length) {
        targets.forEach(el => {
          const nextCfg = Object.assign(readConfig(el), cfg, { slides: cfg.slides });
          if (nextCfg.enabled) {
            saveFillBeforeFx(el);
            suspendFillForFx(el);
          }
          writeConfig(el, nextCfg);
          initBackgroundFx(el);
        });
        persist('special-effects-slide-pause-toggle');
        markSlidesDirtySoon(section, getSelection);
      }
      setStatus(cfg.slides[i]?.paused ? 'Слайд поставлено на паузу' : 'Слайд знову активний');
      return;
    }
    ...
    if (ev.target.closest('[data-apply-fx]')) {
      ev.preventDefault();
      if (!targets.length) { setStatus('Спочатку вибери секцію / контейнер / блок'); return; }
      targets.forEach(el => {
  ...
  document.addEventListener('st:selection-changed', () => syncUi(section, getSelection));
  ...
  // 00987: Main/Header/Footer templates are rendered from SiteFrameStore after
  ...
  const onTemplatesApplied00987 = () => {
    document.querySelectorAll('[data-st-fx-bg-slider]').forEach(initBackgroundFx);
    bootAllBlockCarousels();
    syncUi(section, getSelection);
  };
  window.addEventListener('st:templates-applied', onTemplatesApplied00987);
