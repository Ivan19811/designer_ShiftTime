// js/site-frame/site-frame-edit-layer-bridge-00989.js
...
(() => {
  ...
  const VERSION = '00989-site-frame-main-hero-live-edit-geometry-parity';
  ...
  function startContainerPair(event, owner, dir) {
    ...
    state.active = {
      ...
      minH: resolveSessionFloor00989(getResizeMinHeight(owner), Math.max(1, Math.round(owner.getBoundingClientRect().height || 1)), MIN_H),
  ...
  function startBoxResize(event, owner, dir) {
    ...
    state.active = {
      ...
      minH: resolveSessionFloor00989(measuredMinH, startH, MIN_H),
      directChildHeights: getDirectContentChildren(owner).map((child) => ({
        id: ensureNodeId(child),
        height: Math.round(child.getBoundingClientRect().height || 0),
      })),
  ...
  function onPointerDown(event) {
    ...
    let started = false;
    if (meta.kind === 'container' && owner.parentElement?.classList?.contains('st-row') && /[ew]/.test(dir)) {
