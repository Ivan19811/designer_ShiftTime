// js/site-frame/site-frame-edit-layer-bridge-01027.js
...
(() => {
  ...
  const VERSION = '01027-site-frame-contained-child-resize-parity';
  ...
  // 01027 — Visual relative offsets are not normal-flow geometry.
  // A child moved with position:relative/top/left keeps its original layout slot;
  // therefore its visual getBoundingClientRect() must never inflate the parent's
  // intrinsic/content envelope. Absolute/fixed children do not participate in
  // normal flow at all. Return the layout-flow rect used only for parent sizing.
  function getFlowRect01027(element) {
    if (!isEl(element)) return null;
    try {
      const rect = element.getBoundingClientRect();
      const style = getComputedStyle(element);
      ...
      if (position === 'relative') {
        const leftRaw = normalizedSideValue00991(style.left);
        const rightRaw = normalizedSideValue00991(style.right);
        const topRaw = normalizedSideValue00991(style.top);
        const bottomRaw = normalizedSideValue00991(style.bottom);
        if (leftRaw && leftRaw !== 'auto' && Number.isFinite(parseFloat(leftRaw))) dx = parseFloat(leftRaw);
        else if (rightRaw && rightRaw !== 'auto' && Number.isFinite(parseFloat(rightRaw))) dx = -parseFloat(rightRaw);
        if (topRaw && topRaw !== 'auto' && Number.isFinite(parseFloat(topRaw))) dy = parseFloat(topRaw);
        else if (bottomRaw && bottomRaw !== 'auto' && Number.isFinite(parseFloat(bottomRaw))) dy = -parseFloat(bottomRaw);
      }
  ...
  function getLiveContentHeightFloor00993(element) {
    ...
    try {
      ...
      for (const child of children) {
        ...
        const childRect = getFlowRect01027(child);
  ...
  // 01027 — A child resize is contained by its direct parent content box.
  // Expanding a text/block child must never silently resize ancestors; if the
  // user wants a wider parent/container, that parent is resized explicitly.
  function getExpandableMaxWidth(element, dir) {
    return getMaxWidthInsideParent(element, dir);
  }
  ...
  function growAncestorEnvelopeLive(element, transaction = null) {
    ...
    while (isEl(child) && isEl(parent) && guard++ < 12) {
      ...
      // Absolute/fixed descendants do not own normal-flow height. A relative
      // child's top/bottom visual offset has already been removed above.
      if (!childRect) {
        child = parent;
        parent = parent.parentElement;
        continue;
  ...
  // 01026: an anchored edge in offset mode is a positional edit, not a margin edit.
  // Margins participate in flex/grid layout: reducing margin-top on a stretched flex
  // item can increase its height instead of moving it, and margin >= 0 used to block
  // all travel above/left of the authored origin. Relative insets preserve the
  // element's normal-flow slot and its size while moving the visual box freely in
  // both directions. Store owns position/top/right/bottom/left exactly like any
  // other style property, so F5 and Undo/Redo remain canonical.
  function setElementOffset00991(element, property, value) {
    if (!isEl(element) || !property) return false;
    const numeric = Number(value);
    const safe = Math.round(clamp(Number.isFinite(numeric) ? numeric : 0, -MAX_SIZE, MAX_SIZE));
    const style = getComputedStyle(element);
    const current = Math.round(numPx(style.getPropertyValue(property)) || 0);
    const position = normalizedSideValue00991(style.position);
    const needsRelative = !['relative','absolute','fixed','sticky'].includes(position);
    if (Math.abs(safe - current) <= 1 && !needsRelative) return false;
    const patch = { [property]: `${safe}px` };
    if (needsRelative) patch.position = 'relative';
    getAuthority().previewStyle(element, patch);
    return true;
  }
  ...
  function startContainerPair(event, owner, dir) {
    ...
    state.active = {
      ...
      startInsetLeft: numPx(ownerStyle00991.left),
      startInsetRight: numPx(ownerStyle00991.right),
      startInsetTop: numPx(ownerStyle00991.top),
      startInsetBottom: numPx(ownerStyle00991.bottom),
      items,
      widths,
      mins,
      index,
  ...
  function applyResizeFrame(active, clientX, clientY) {
    ...
    if (active.type === 'grid-pair' || active.type === 'flex-pair') {
      ...
      if (vertical) {
        if (active.verticalMode === 'offset') {
          const property = active.anchorY === 'bottom' ? 'bottom' : 'top';
          const rawOffset = active.anchorY === 'bottom' ? active.startInsetBottom - dy : active.startInsetTop + dy;
    ...
    if (horizontal) {
      if (active.horizontalMode === 'offset') {
        const property = active.anchorX === 'right' ? 'right' : 'left';
        const rawOffset = active.anchorX === 'right'
          ? active.startInsetRight - dx
          : active.startInsetLeft + dx;
        ...
        if (setElementWidth(owner, clamp(rawWidth, active.minW, active.maxW), active.minW, active.flexWidth)) {
          active.changedIds.add(ensureNodeId(owner));
          active.horizontalChanged = true;
          // 01027: horizontal child resizing is contained. Never grow ancestor
          // widths as a side effect of resizing a nested text/block/container.
    ...
    if (vertical) {
      if (active.verticalMode === 'offset') {
        ...
        const rawOffset = active.anchorY === 'bottom'
          ? active.startInsetBottom - dy
          : active.startInsetTop + dy;
        if (setElementOffset00991(owner, property, rawOffset)) {
          active.changedIds.add(ensureNodeId(owner));
          active.verticalChanged = true;
        }
  ...
  function onPointerUp(event) {
    ...
    log('resize-end', {
      ...
      liveParentGrowMeasurement: 'active-child-flow-bottom-only-01027',
      containedChildWidth01027: true,
      relativeOffsetsExcludedFromParentEnvelope01027: true,
      ancestorWidthAutoGrow01027: false,
      ...
      relativePositionOffsets01026: true,
      signedOffsets01026: true,
      containedChildWidth01027: true,
      relativeOffsetsExcludedFromParentEnvelope01027: true,
      ancestorWidthAutoGrow01027: false,
  ...
  function boot() {
    ...
    window.ST_SITE_FRAME_EDIT_LAYER_00882 = Object.freeze({
      ...
      oppositeSideChangesSize: true,
        ...
        anchoredSideChangesOffset: true,
        oppositeSideChangesSize: true,
        offsetProperties: Object.freeze(['left','right','top','bottom']),
        offsetPositionMode01026: 'relative-inset-preserves-flow-size',
