...
export class SiteFrameDomRenderer {
  ...
  renderNode(nodeId, exactElement = null) {
    ...
    const geometryOwnedStyleKeys01011 = new Set();
    if (geometry.widthOwned === true && typeof box.width === 'number' && Number.isFinite(box.width)) {
      geometryOwnedStyleKeys01011.add('width');
      geometryOwnedStyleKeys01011.add('min-width');
      geometryOwnedStyleKeys01011.add('max-width');
    }
    if (geometry.heightOwned === true && typeof box.height === 'number' && Number.isFinite(box.height)) {
      geometryOwnedStyleKeys01011.add('height');
      geometryOwnedStyleKeys01011.add('min-height');
      geometryOwnedStyleKeys01011.add('max-height');
    }

    for (const [property, rawValue] of Object.entries(effectiveStyle)) {
      if (!property) continue;
      // 01011: once resize owns an axis, node.style must not replay the authored
      // width/height after applyBox has just rendered the Store-owned geometry.
      // This was the concrete Main Hero failure: width changed in JSON, then the
      // authored `width:min(570px,100%)` was written again in the same render pass,
      // so the box stayed 570px while text reflow only changed its height.
      if (geometryOwnedStyleKeys01011.has(property)) continue;
