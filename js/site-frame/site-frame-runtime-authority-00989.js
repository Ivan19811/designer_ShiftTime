...
import { SiteFrameDomRenderer } from './site-frame-renderer.js?v=00961';
...
const VERSION = '00961-site-frame-main-template-residual-preview-parity';
...
function nodeFromElement(element, parentId, area, previousNode = null) {
  ...
  const manualWidth = pxNumber(element.dataset.sfManualW);
  const manualHeight = pxNumber(element.dataset.sfManualH);
  const previousGeometry = previousNode?.meta?.geometry || {};
  const widthOwned = manualWidth > 0 || previousGeometry.widthOwned === true;
  const heightOwned = manualHeight > 0 || previousGeometry.heightOwned === true;
...
function mainElementForNode(node) {
  ...
  applyMainNodeStyle(element, node);
  if (node.kind === KIND.BLOCK) {
    if (isMainEditableTextBlock00901(node)) renderMainTextBlock00901(element, node);
...
function ownedNodeIds() {
  return store.all()
    .filter(node => node.meta?.geometry?.widthOwned === true || node.meta?.geometry?.heightOwned === true || node.meta?.geometry?.gridOwned === true)
    .map(node => node.id);
}
...
function beginTransaction(config = {}) {
  ...
  log('transaction-begin', { id: transaction.id, ownerId: ownerNode.id, area: activeContext.area, type: activeContext.type });
  return transaction;
}
...
function previewBox(element, patch = {}) {
  ...
  const changedIds = store.updateNodes({
    [node.id]: {
      box: nextBox,
      style,
      meta: { ...(node.meta || {}), geometry },
    },
  }, { emit: false, rebuildTreeMeta: false });
  store.markTransactionChanges(changedIds);
  // 00928: render the exact gesture owner. A global id lookup can resolve a
  // persisted/template occurrence instead of the visible Main node.
  renderer.renderNode(node.id, element);
  return changedIds;
}
...
function previewPair(config = {}) {
  ...
  if (config.type === 'grid-pair') {
    const rowNode = ensureNode(config.row);
    patches[rowNode.id] = {
    ...
    renderTargets.push([rowNode.id, config.row]);
    pairIndexes.forEach((index) => {
      const node = ensureNode(items[index]);
      patches[node.id] = { box: { ...(node.box || {}), width: Math.round(widths[index]) } };
    });
    ...
    pairIndexes.forEach((index) => {
      ...
      renderTargets.push([node.id, items[index]]);
    });
  } else {
...
function commitMainTypography00921(element, reason = 'main-typography-controls-00921') {
  ...
  const same = String(node.content?.html ?? '') === html
    ...
    && String(node.meta?.textEditableStyle || '').trim() === textEditableStyle;
  ...
  const changedIds = store.updateNodes({
    [id]: { content: nextContent, meta: nextMeta },
  }, { emit: false, rebuildTreeMeta: false });
  store.markTransactionChanges(changedIds);
