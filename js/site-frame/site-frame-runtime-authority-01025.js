...
import { SiteFrameDomRenderer } from './site-frame-renderer-01011.js?v=01011';
...
const VERSION = '01041-commerce-component-template-store-parity';
...
function sanitizedLeafContent(element) {
  ...
  clone.querySelectorAll('.st-resize,.st-selection-ui,.st-drag-marker,.st-drop-marker,.hb-resize-handle,.st-fill-filter-layer,[data-st-fill-visual-layer="filter"]').forEach(node => node.remove());
  return {
...
// 01025 persisted-state migration for trees imported by the faulty 01024 rule.
// This is a one-time canonical JSON migration, not a DOM repair: old template
// LEVEL nodes that had no explicit column intent were saved as `column` solely
// because the importer default was wrong. Respect any explicit current/authored
// flex-direction:column so a real user/authored choice is never overwritten.
function migrateImportedMainLevelLayout01025() {
  let changed = 0;
  const changedIds = [];
  for (const node of store.all()) {
    if (!node || node.area !== 'main' || node.kind !== KIND.LEVEL) continue;
    if (String(node.layout?.mode || '') !== 'column') continue;
    if (!String(node.meta?.templateId || '').trim()) continue;
    if (String(node.meta?.styleCascade00960 || '') !== 'template-authored-until-edited') continue;

    const currentStyle = node.style && typeof node.style === 'object' ? node.style : {};
    const authoredStyle = node.meta?.authoredStyle00960 && typeof node.meta.authoredStyle00960 === 'object'
      ? node.meta.authoredStyle00960
      : {};
    const currentOrient = String(currentStyle['flex-direction'] || '').trim().toLowerCase();
    const authoredOrient = String(authoredStyle['flex-direction'] || '').trim().toLowerCase();
    if (currentOrient === 'column' || authoredOrient === 'column') continue;

    node.layout = { ...(node.layout || {}), mode: 'row' };
    node.meta = {
      ...(node.meta || {}),
      layoutParity01025: 'level-default-row-migrated',
      layoutParityMigratedAt01025: Date.now(),
    };
    changed += 1;
    changedIds.push(node.id);
  }
  if (changed) {
    try { store.rebuildTreeMeta(); } catch {}
    log('main-level-layout-migrated-01025', {
      changed,
      changedIds: changedIds.slice(0, 80),
      canonicalLevelDefault: 'row',
      explicitColumnPreserved: true,
      domRepair: false,
    });
  }
  return Object.freeze({ changed, changedIds });
}
...
function applyMainFillStylePriority00999(element, node) {
  if (!(element instanceof HTMLElement) || !node) return;
  const priorities = node.meta?.fill?.stylePriority && typeof node.meta.fill.stylePriority === 'object'
    ? node.meta.fill.stylePriority
    : null;
  if (!priorities) return;
  for (const [property, priority] of Object.entries(priorities)) {
    if (!property || !priority) continue;
    const value = node.style?.[property];
    if (value == null || value === '') continue;
    element.style.setProperty(property, String(value), String(priority));
  }
}

function ensureMainFillFilterLayer00999(element) {
  if (!(element instanceof HTMLElement)) return null;
  if (!element.classList.contains('st-block-surfacefx') || !element.classList.contains('st-bgfx')) return null;
  let layer = Array.from(element.children || []).find((child) => child instanceof HTMLElement && child.classList.contains('st-fill-filter-layer'));
  if (!(layer instanceof HTMLElement)) {
    layer = document.createElement('span');
    layer.className = 'st-fill-filter-layer';
    layer.setAttribute('data-st-fill-visual-layer', 'filter');
...
function safeAuthoredTag01024(node) {
  const raw = String(node?.meta?.dom?.tag || '').trim().toLowerCase();
  const allowed = new Set(['section','div','span','p','small','strong','b','em','i','h1','h2','h3','h4','h5','h6','details','summary','a','button','ul','ol','li','figure','figcaption','article','aside','nav','header','footer']);
  if (allowed.has(raw)) return raw;
  return node?.kind === KIND.SECTION ? 'section' : 'div';
}

function mainElementForNode(node) {
  const visualNode00991 = getEffectiveResponsiveNode00991(node) || node;
  const element = document.createElement(safeAuthoredTag01024(node));
  ...
  const authoredId01024 = String(node.meta?.dom?.id || '').trim();
  if (authoredId01024) element.id = authoredId01024;
  const authoredAttributes01024 = node.meta?.dom?.attributes && typeof node.meta.dom.attributes === 'object'
    ? node.meta.dom.attributes
    : null;
  if (authoredAttributes01024) {
    for (const [name, value] of Object.entries(authoredAttributes01024)) {
      if (!name || name === 'class' || name === 'style' || name === 'id' || name.startsWith('data-')) continue;
      try { element.setAttribute(name, String(value ?? '')); } catch {}
    }
  }
  ...
  applyMainFillStylePriority00999(element, visualNode00991);
  if (node.kind === KIND.BLOCK) {
    if (node.componentType === 'authored-html' || node.meta?.authoredPrimitive01024 === true) {
      if (node.content?.html) element.innerHTML = String(node.content.html);
      else element.textContent = String(node.content?.text || '');
    }
    else if (isMainEditableTextBlock00901(node)) renderMainTextBlock00901(element, visualNode00991);
  ...
  ensureMainFillFilterLayer00999(element);
  return element;
}
...
const MAIN_AREA_SNAPSHOT_VERSION_01040 = 'st-main-area-snapshot-v1-01040';

function cloneJson01040_(value, fallback = null) {
  try { return JSON.parse(JSON.stringify(value)); } catch { return fallback; }
}

function exportMainAreaSnapshot01040() {
  const area = store.findArea('main');
  if (!area) {
    return Object.freeze({
      version: MAIN_AREA_SNAPSHOT_VERSION_01040,
      rootIds: Object.freeze([]),
      nodes: Object.freeze({}),
      areaMeta: Object.freeze({}),
    });
  }
  const nodes = {};
  const visit = (id) => {
    const node = store.maybeGet(id);
    if (!node || node.area !== 'main') return;
    nodes[id] = cloneJson01040_(node, {});
    for (const childId of node.children || []) visit(childId);
  };
  const rootIds = [...(area.children || [])];
  for (const id of rootIds) visit(id);
  return Object.freeze({
    version: MAIN_AREA_SNAPSHOT_VERSION_01040,
    rootIds: Object.freeze(rootIds),
    nodes: Object.freeze(nodes),
    areaMeta: Object.freeze(cloneJson01040_(area.meta || {}, {})),
  });
}

function restoreMainAreaSnapshot01040(snapshot = null, options = {}) {
  if (store.hasActiveTransaction()) {
    return Object.freeze({ ok: false, reason: 'active-site-frame-transaction', rootCount: store.findArea('main')?.children?.length || 0 });
  }
  const reason = String(options.reason || 'page-main-snapshot-restore-01040');
  const state = store.toJSON();
  const mainAreaEntry = Object.entries(state.nodes || {}).find(([, node]) => node?.kind === KIND.AREA && node?.area === 'main');
  if (!mainAreaEntry) return Object.freeze({ ok: false, reason: 'main-area-missing', rootCount: 0 });
  const [mainAreaId, mainArea] = mainAreaEntry;

  // Main is page-scoped. Remove only the current Main subtree; Header/Footer and
  // root-level SiteFrame metadata stay untouched.
  for (const [id, node] of Object.entries(state.nodes || {})) {
    if (id !== mainAreaId && node?.area === 'main') delete state.nodes[id];
  }

  const validSnapshot = snapshot && typeof snapshot === 'object'
    && snapshot.version === MAIN_AREA_SNAPSHOT_VERSION_01040
    && snapshot.nodes && typeof snapshot.nodes === 'object';
  const sourceNodes = validSnapshot ? snapshot.nodes : {};
  const rootIds = validSnapshot && Array.isArray(snapshot.rootIds) ? snapshot.rootIds.map(String) : [];

  mainArea.children = [];
  mainArea.meta = validSnapshot ? cloneJson01040_(snapshot.areaMeta || {}, {}) : {};

  for (const [id, rawNode] of Object.entries(sourceNodes)) {
    if (!rawNode || typeof rawNode !== 'object') continue;
    const node = cloneJson01040_(rawNode, null);
    if (!node) continue;
    node.id = String(node.id || id);
    node.area = 'main';
    state.nodes[node.id] = node;
  }
  mainArea.children = rootIds.filter((id) => !!state.nodes[id]);
  for (const id of mainArea.children) {
    state.nodes[id].parentId = mainAreaId;
    state.nodes[id].area = 'main';
  }

  store.load(state, { emit: false });
  // History is not shared across page tabs. Keeping another page's undo stack
  // would allow Ctrl+Z to restore a different page's Main.
  try { store.clearHistory?.(); } catch {}
  const mainRendered = renderMainSelectionArea(reason);
  const geometryRendered = renderer.renderArea('main');
  if (options.persist !== false) {
    persistStore(reason);
    try { persistHistory(`${reason}:history-clear`); } catch {}
  }
  clearPublishedSelection00899();
  updateHistoryButtons00899();
  log('page-main-snapshot-restored-01040', {
    reason,
    validSnapshot,
    rootCount: store.findArea('main')?.children?.length || 0,
    nodeCount: Object.keys(sourceNodes).length,
    mainRendered,
    geometryRendered,
    pageScoped: true,
    historyCrossPage: false,
  });
  return Object.freeze({
    ok: true,
    reason,
...
function emptyMainAreaSnapshot01040() {
  return Object.freeze({
    version: MAIN_AREA_SNAPSHOT_VERSION_01040,
    rootIds: Object.freeze([]),
    nodes: Object.freeze({}),
...
function authoredAttributesFromElement01024(element) {
  const out = {};
  for (const attr of Array.from(element?.attributes || [])) {
    const name = String(attr?.name || '').trim();
    if (!name || name === 'class' || name === 'style' || name === 'id' || name.startsWith('data-')) continue;
    // Runtime editor state is regenerated; semantic authored attributes are retained.
    if (['contenteditable','draggable','spellcheck'].includes(name)) continue;
    out[name] = String(attr?.value ?? '');
  }
  return out;
}

function isExplicitTemplateStructure01024(element) {
  if (!(element instanceof HTMLElement)) return false;
  return element.matches('section,.st-section,.st-row,[data-st-node="level"],.st-block,[data-st-node="container"]');
}

function isTemplateRuntimeArtifact01024(element) {
  if (!(element instanceof HTMLElement)) return true;
  return element.matches('.st-resize,.st-selection-ui,.st-drag-marker,.st-drop-marker,.hb-resize-handle,.st-fill-filter-layer,[data-st-fill-visual-layer="filter"]');
}

// 01024: every direct authored HTMLElement under a structural SiteFrame node must
// survive Template -> Store -> Renderer. Previously only .st-row/.st-block children
// were visited, so plain authored wrappers such as <div>, <strong>, <p>, <details>
// disappeared after Apply even though Preview still showed them.
function directTemplateImportChildren01024(element) {
  return Array.from(element.children || []).filter((child) => child instanceof HTMLElement && !isTemplateRuntimeArtifact01024(child));
}

function directTemplateStructureChildren00888(element) {
  return directTemplateImportChildren01024(element).filter((child) => isExplicitTemplateStructure01024(child));
}
...
function inferTemplateKind00888(element) {
  ...
  // Plain authored descendants are promoted to opaque SiteFrame blocks rather
  // than silently discarded. Their exact tag/class/innerHTML is stored in JSON.
  return KIND.BLOCK;
}
...
function componentTypeFromTemplate00888(element, kind) {
  ...
  const authoredComponent = semanticPriority.find((component) => authored.has(component));
  if (authoredComponent) return authoredComponent;
  const explicitKind = String(element.dataset.blockKind || '').trim().toLowerCase();
  if (explicitKind) return explicitKind;
  // 01024: a plain authored element is an opaque HTML leaf. This preserves
  // semantic wrappers such as <details>/<summary>, <strong>, <p>, <small>, etc.
  if (!isExplicitTemplateStructure01024(element)) return 'authored-html';
  return 'text';
}

function templateNodeInput00888(element, kind, id, parentId, templateId) {
  const style = styleObjectFromElement00888(element);
  const authoredPrimitive01024 = kind === KIND.BLOCK && !isExplicitTemplateStructure01024(element);
  const authoredEditable01024 = kind === KIND.BLOCK
    ? element.querySelector?.(':scope > .st-text-edit,:scope > [data-st-text-target="1"],:scope > [contenteditable="true"],:scope > [contenteditable="plaintext-only"]')
    : null;
  const authoredTextEditableStyle01024 = authoredEditable01024 instanceof HTMLElement
    ? String(authoredEditable01024.getAttribute('style') || '').trim()
    : '';
  ...
  // 01025: preserve the canonical SiteFrame meaning of a LEVEL when authored
  // layout is not inline. The 01024 importer treated an empty display/orient as
  // `column`, which contradicted the SiteFrame contract (LEVEL defaults to row).
  // That silently turned CSS-authored flex rows into columns after Apply. A real
  // example was the engraving banner: external CSS supplied display:flex and
  // align-items:center, while the importer injected flex-direction:column, so
  // the copy became horizontally centered and its left offset handle could not
  // travel back to the authored left edge.
  //
  // Explicit authored layout still wins:
  //   grid                 -> row-like structural level
  //   flex + column        -> column
  //   flex + row/default   -> row
  //   data-layout-orient   -> exact row/column intent
  // Otherwise use the canonical kind default: LEVEL=row, CONTAINER=column.
  let authoredLayoutMode01025 = 'none';
  if (kind === KIND.LEVEL || kind === KIND.CONTAINER) {
    if (display.includes('grid')) authoredLayoutMode01025 = 'row';
    else if (orient === 'column') authoredLayoutMode01025 = 'column';
    else if (orient === 'row') authoredLayoutMode01025 = 'row';
    else if (display.includes('flex')) authoredLayoutMode01025 = 'row';
    else authoredLayoutMode01025 = kind === KIND.LEVEL ? 'row' : 'column';
  }
  node.layout = {
    mode: authoredLayoutMode01025,
  ...
  node.meta = {
    ...
    dom: {
      ...
      attributes: authoredAttributesFromElement01024(element),
    },
    ...
    textEditableStyle: authoredTextEditableStyle01024,
    authoredPrimitive01024,
    structuralImport01024: authoredPrimitive01024 ? 'promoted-authored-html' : 'explicit-site-frame-node',
    geometry: { widthOwned: false, heightOwned: false, gridOwned: false }
  };
  return node;
}
...
function importMainTemplateSection00888(html, parentId, templateId) {
  ...
  const visit = (element, targetParentId) => {
    ...
    createdIds.push(id);
    if (kind !== KIND.BLOCK) {
      for (const child of directTemplateImportChildren01024(element)) visit(child, id);
    }
    return id;
  };
  const rootId = visit(source, parentId);
  const authoredPrimitiveIds01024 = createdIds.filter((id) => store.maybeGet(id)?.meta?.authoredPrimitive01024 === true);
  log('main-template-structural-import-01024', {
    templateId: String(templateId || ''),
    createdNodes: createdIds.length,
    promotedAuthoredPrimitiveNodes: authoredPrimitiveIds01024.length
  });
  return Object.freeze({ rootId, createdIds: Object.freeze([...createdIds]), authoredPrimitiveIds01024: Object.freeze([...authoredPrimitiveIds01024]) });
}


function commerceComponentTypeFromNode01041(node) {
  const dataset = node?.meta?.dom?.dataset && typeof node.meta.dom.dataset === 'object' ? node.meta.dom.dataset : {};
  const raw = String(dataset.commerceComponent || dataset.shopType || node?.componentType || '').trim().toLowerCase();
  return raw.startsWith('commerce:') ? raw.slice('commerce:'.length) : raw;
}

function isMainComponentInsertHost01041(node) {
  return !!node && node.area === 'main' && (node.kind === KIND.CONTAINER || node.kind === KIND.LEVEL);
}

function firstMainComponentInsertHost01041(startId) {
  const start = store.maybeGet(String(startId || ''));
  if (!start) return '';
  let firstLevel = '';
  const visit = (node) => {
    if (!node) return '';
    if (node.kind === KIND.CONTAINER) return node.id;
    if (!firstLevel && node.kind === KIND.LEVEL) firstLevel = node.id;
    for (const childId of node.children || []) {
      const found = visit(store.maybeGet(childId));
      if (found) return found;
    }
    return '';
  };
  return visit(start) || firstLevel;
}

function resolveMainComponentTarget01041(componentType = 'product-card') {
  const wantedType = String(componentType || 'product-card').trim().toLowerCase();
  ...
  push(selected?.nodeId);
  push(window.__ST_SITE_FRAME_MAIN_ACTIVE_00887?.nodeId);
  const mainSlot = document.getElementById('st-site-main-slot');
  const liveSelected = mainSlot?.querySelector?.('.sf-selection-current.is-selected,.sf-selection-current,[data-sf-id].is-selected,[data-node-id].is-selected') || null;
  if (liveSelected instanceof HTMLElement) push(liveSelected.dataset?.nodeId || liveSelected.dataset?.hbRef || liveSelected.dataset?.sfId || liveSelected.id);

  for (const id of candidates) {
    const node = store.maybeGet(id);
    if (!node || node.area !== 'main') continue;
    if (node.kind === KIND.BLOCK) {
      const parent = store.maybeGet(node.parentId);
      const selectedCommerceType01050 = commerceComponentTypeFromNode01041(node);
      return Object.freeze({
        selectedId: node.id,
        insertParentId: isMainComponentInsertHost01041(parent) ? parent.id : '',
        replaceTargetId: selectedCommerceType01050 === wantedType ? node.id : '',
        replaceCommerceTargetId01050: selectedCommerceType01050 ? node.id : '',
        selectedCommerceType01050,
        selectedKind: node.kind,
        source: 'explicit-selection-01041'
      });
    }
    if (isMainComponentInsertHost01041(node)) {
      return Object.freeze({ selectedId: node.id, insertParentId: node.id, replaceTargetId: '', selectedKind: node.kind, source: 'explicit-selection-01041' });
    }
    if (node.kind === KIND.SECTION) {
      const hostId = firstMainComponentInsertHost01041(node.id);
      if (hostId) return Object.freeze({ selectedId: node.id, insertParentId: hostId, replaceTargetId: '', selectedKind: node.kind, source: 'selected-section-descendant-01041' });
    }
  }

  const area = store.findArea('main');
  if (area && Array.isArray(area.children) && area.children.length === 1) {
    const hostId = firstMainComponentInsertHost01041(area.children[0]);
    if (hostId) return Object.freeze({ selectedId: '', insertParentId: hostId, replaceTargetId: '', selectedKind: '', source: 'single-main-root-default-host-01041' });
  }
  return Object.freeze({ selectedId: '', insertParentId: '', replaceTargetId: '', selectedKind: '', source: 'no-valid-target-01041' });
}

function importMainComponentTemplate01041(html, parentId, templateId, componentType) {
  const host = document.createElement('div');
  host.innerHTML = String(html || '').trim();
  const wanted = String(componentType || '').trim();
  const source = (wanted ? host.querySelector(`[data-commerce-component="${wanted}"]`) : null) || host.querySelector('[data-commerce-component],.st-block');
  if (!(source instanceof HTMLElement)) throw new Error('Commerce component template must contain a component root');
  if (!source.classList.contains('st-block')) throw new Error('Commerce component root must be a .st-block');
  const stamp = Date.now().toString(36);
  const id = `sf_main_cmp_${stamp}_${Math.random().toString(36).slice(2,7)}`;
  const node = templateNodeInput00888(source, KIND.BLOCK, id, parentId, templateId);
  node.componentType = `commerce:${String(componentType || source.dataset.commerceComponent || 'component').trim().toLowerCase()}`;
  node.meta = {
    ...(node.meta || {}),
    commerceComponent01041: true,
    commerceComponentType01041: String(componentType || source.dataset.commerceComponent || '').trim().toLowerCase(),
    commerceTemplateId01041: String(templateId || ''),
    storeOwned01041: true
  };
  store.addNode(parentId, node);
  return Object.freeze({ rootId: id, createdIds: Object.freeze([id]) });
}

function applyMainComponentTemplate01041({ html = '', templateId = '', componentType = 'product-card', mode = 'add', parentNodeId = '', targetNodeId = '', allowTypeChange01050 = false } = {}) {
  const requestedMode = String(mode || 'add').toLowerCase() === 'replace' ? 'replace' : 'add';
  const wantedType = String(componentType || 'product-card').trim().toLowerCase();
  const captured = resolveMainComponentTarget01041(wantedType);
  let parentId = String(parentNodeId || '').trim();
  let replaceId = String(targetNodeId || '').trim();
  let replaceIndex = -1;
  let preservedCommerceBinding01064 = null;

  if (requestedMode === 'replace') {
    const target = store.maybeGet(replaceId || captured.replaceTargetId || (allowTypeChange01050 ? captured.replaceCommerceTargetId01050 : ''));
    const currentType01050 = commerceComponentTypeFromNode01041(target);
    const validType01050 = !!currentType01050 && (currentType01050 === wantedType || allowTypeChange01050 === true);
    if (!target || target.area !== 'main' || target.kind !== KIND.BLOCK || !validType01050) {
      return Object.freeze({ ok: false, reason: 'commerce-component-replace-target-required', requestedMode, componentType: wantedType, allowTypeChange01050: !!allowTypeChange01050 });
    }
    replaceId = target.id;
    const replacedComponentType01050 = currentType01050;
    // 01064: replacing only the visual template of the same Commerce Card type must
    // preserve its Marketplace entity binding. Product→Category type changes do NOT
    // carry the old entity id because the canonical entity domains differ.
    if (replacedComponentType01050 === wantedType) {
      const ds01064 = target?.meta?.dom?.dataset && typeof target.meta.dom.dataset === 'object' ? target.meta.dom.dataset : {};
      const keep01064 = {};
      ['commerceBindingSource01064','commerceBindingSource','commerceEntityId01064','commerceEntityId','commerceBindingVersion01064','commerceBindingMode01067','commerceBindingVersion01067','commerceCollectionSource01067','commerceCollectionCategoryId01067','commerceCollectionCollectionId01067','commerceCollectionManualProductIds01067','commerceCollectionIncludeDescendants01067','commerceCollectionStatus01067','commerceCollectionAvailability01067','commerceCollectionSort01067','commerceCollectionLimit01067','commerceCollectionOverflow01067','commerceCollectionColsDesktop01067','commerceCollectionColsTablet01067','commerceCollectionColsMobile01067','commerceCollectionColsSmall01067','commerceCollectionGap01067'].forEach((key01064) => {
        const value01064 = String(ds01064?.[key01064] || '').trim();
        if (value01064) keep01064[key01064] = value01064;
      });
      if (Object.keys(keep01064).length) preservedCommerceBinding01064 = keep01064;
    }
    const parent = store.maybeGet(target.parentId);
    if (!isMainComponentInsertHost01041(parent)) return Object.freeze({ ok: false, reason: 'commerce-component-parent-invalid', requestedMode, componentType: wantedType });
    parentId = parent.id;
    replaceIndex = parent.children.indexOf(target.id);
  } else {
    const parent = store.maybeGet(parentId || captured.insertParentId);
    if (!isMainComponentInsertHost01041(parent)) {
      return Object.freeze({ ok: false, reason: 'commerce-component-insert-target-required', requestedMode, componentType: wantedType });
    }
    parentId = parent.id;
  }

  store.beginTransaction('main-commerce-component-template-01041', { requestedMode, templateId, componentType: wantedType, parentId, replaceId, allowTypeChange01050: !!allowTypeChange01050 });
  let committed = false;
  try {
    if (requestedMode === 'replace' && replaceId && store.maybeGet(replaceId)) store.removeNode(replaceId);
    const imported = importMainComponentTemplate01041(html, parentId, templateId, wantedType);
    if (preservedCommerceBinding01064) {
      const newNode01064 = store.maybeGet(imported.rootId);
      if (newNode01064) {
        const dom01064 = newNode01064.meta?.dom && typeof newNode01064.meta.dom === 'object' ? newNode01064.meta.dom : {};
        store.updateNode(imported.rootId, { meta: { ...(newNode01064.meta || {}), dom: { ...dom01064, dataset: { ...(dom01064.dataset || {}), ...preservedCommerceBinding01064 } }, commerceBindingPreserved01064: true } });
      }
    }
    if (replaceIndex >= 0) store.moveNode(imported.rootId, parentId, replaceIndex);
    store.markTransactionChanges([parentId, ...imported.createdIds]);
    const tx = store.commitTransaction({ reason: 'commerce-component-template-01041', requestedMode, templateId, componentType: wantedType, newId: imported.rootId, replacedId: replaceId, allowTypeChange01050: !!allowTypeChange01050 });
    committed = true;
    renderMainSelectionArea('commerce-component-template-01041');
    persistStore('commerce-component-template-01041');
    const slot = document.getElementById('st-site-main-slot');
    const rendered = slot?.querySelector?.(`[data-node-id="${imported.rootId}"],[data-hb-ref="${imported.rootId}"],[data-sf-id="${imported.rootId}"]`) || null;
    if (rendered instanceof HTMLElement) {
      publishSiteFrameSelection({ area: 'main', slot, target: rendered, kind: 'block', reason: 'commerce-component-template-01041' });
    }
    try { window.ST_SAVE_ROOT_DOM_HTML?.({ draft: false, reason: 'commerce-component-template-01041', preserveLiveMain: true }); } catch {}
    try { window.dispatchEvent(new CustomEvent('st:commerce-component-template-applied-01041', { detail: { requestedMode, templateId, componentType: wantedType, newId: imported.rootId, replacedId: replaceId, parentId, allowTypeChange01050: !!allowTypeChange01050 } })); } catch {}
    try { window.dispatchEvent(new CustomEvent('builder:structureChanged', { detail: { reason: 'commerce-component-template-01041', templateId, newId: imported.rootId } })); } catch {}
    log('commerce-component-template-applied-01041', { requestedMode, templateId, componentType: wantedType, newId: imported.rootId, replacedId: replaceId, parentId, allowTypeChange01050: !!allowTypeChange01050, targetSource: captured.source, storeAuthority: true, directDomInsert: false });
    return Object.freeze({ ok: true, requestedMode, templateId, componentType: wantedType, newId: imported.rootId, replacedId: replaceId, parentId, transactionId: tx.id });
  } catch (error) {
    if (!committed) {
      try { store.rollbackTransaction({ reason: 'commerce-component-template-error-01041', message: String(error?.message || error || '') }); } catch {}
    }
    log('commerce-component-template-error-01041', { requestedMode, templateId, componentType: wantedType, message: String(error?.message || error || '') }, 'error');
    return Object.freeze({ ok: false, reason: String(error?.message || error || 'commerce-component-template-error'), requestedMode, componentType: wantedType });
  }
}

function commerceBindingDatasetFromElement01067(root) {
  const dataset = authoredDatasetFromElement00987(root);
  ['commerceBindingState01064','commerceBindingEntityName01064','commerceCollectionState01067','commerceCollectionRendered01067','commerceCollectionTotal01067','commerceRuntimeClone01067'].forEach((key) => { delete dataset[key]; });
  return dataset;
}

function commitMainComponentBindingDataset01067(element, reason = 'commerce-binding-dataset-01067') {
  const input = element instanceof HTMLElement ? element : null;
  const root = input?.closest?.('#st-site-main-slot [data-sf-id].st-block,#st-site-main-slot [data-node-id].st-block') || null;
  if (!(root instanceof HTMLElement)) return Object.freeze({ ok: false, reason: 'no-store-owned-main-component' });
  const nodeId = String(root.dataset.sfId || root.dataset.nodeId || root.dataset.stNodeId || '').trim();
  const node = store.maybeGet(nodeId);
  if (!node || node.area !== 'main' || node.kind !== KIND.BLOCK) return Object.freeze({ ok: false, reason: 'component-node-not-found' });
  const componentType = String(root.dataset.commerceComponent || commerceComponentTypeFromNode01041(node) || '').trim().toLowerCase();
  if (!componentType) return Object.freeze({ ok: false, reason: 'not-commerce-component' });
  store.beginTransaction('main-commerce-binding-dataset-01067', { nodeId, componentType, reason });
  try {
    const domMeta = node.meta?.dom && typeof node.meta.dom === 'object' ? node.meta.dom : {};
    store.updateNode(nodeId, {
      meta: {
        ...(node.meta || {}),
        dom: { ...domMeta, dataset: commerceBindingDatasetFromElement01067(root) },
        commerceComponent01041: true,
        commerceComponentType01041: componentType,
        lastCommerceBindingDatasetCommit01067: Date.now()
      }
    });
    store.markTransactionChanges([nodeId]);
    const tx = store.commitTransaction({ reason, nodeId, componentType, oneCommit: true, datasetOnly01067: true });
    persistStore(reason);
    try { window.ST_SAVE_ROOT_DOM_HTML?.({ draft: false, reason, preserveLiveMain: true }); } catch {}
    log('commerce-binding-dataset-committed-01067', { nodeId, componentType, reason, transactionId: tx.id, datasetOnly01067: true, liveMarketplaceValuesPersisted: false });
    return Object.freeze({ ok: true, nodeId, componentType, transactionId: tx.id, datasetOnly01067: true });
  } catch (error) {
    try { store.rollbackTransaction({ reason, message: String(error?.message || error || '') }); } catch {}
    return Object.freeze({ ok: false, reason: String(error?.message || error || 'commerce-binding-dataset-error') });
  }
}

function commitMainComponentContent01041(element, reason = 'commerce-component-content-01041') {
  const input = element instanceof HTMLElement ? element : null;
  const root = input?.closest?.('#st-site-main-slot [data-sf-id].st-block,#st-site-main-slot [data-node-id].st-block') || null;
  if (!(root instanceof HTMLElement)) return Object.freeze({ ok: false, reason: 'no-store-owned-main-component' });
  const nodeId = String(root.dataset.sfId || root.dataset.nodeId || root.dataset.stNodeId || '').trim();
  ...
  const componentType = String(root.dataset.commerceComponent || commerceComponentTypeFromNode01041(node) || '').trim().toLowerCase();
  if (!componentType) return Object.freeze({ ok: false, reason: 'not-commerce-component' });
  store.beginTransaction('main-commerce-component-content-01041', { nodeId, componentType, reason });
  try {
    const domMeta = node.meta?.dom && typeof node.meta.dom === 'object' ? node.meta.dom : {};
    const contentClone01041 = root.cloneNode(true);
    contentClone01041.querySelectorAll('.st-resize,.st-selection-ui,.st-drag-marker,.st-drop-marker,.hb-resize-handle,.st-resize-handle,.st-section-handle,.st-block-handle,.st-drag-handle,.st-col-resizer,.st-sec-resizer,[data-st-fill-visual-layer="filter"],.st-fill-filter-layer,.st-commerce-grid-runtime-01067,[data-commerce-grid-runtime01067],[data-commerce-runtime-clone-01067]').forEach((artifact) => artifact.remove());
    const rootStyle01046 = styleObjectFromElement00888(root);
    store.updateNode(nodeId, {
      // 01046: commerce component root style is Store-owned too. Earlier component
      // commits persisted only innerHTML + authored dataset, so a layout widget
      // could change the live component width/height and lose those root-level
      // structural styles on the next render/F5. Persist the live root inline
      // style into node.style; authoredStyle00960 remains the immutable template
      // baseline and is intentionally not rewritten.
      style: { ...rootStyle01046 },
      content: { ...(node.content || {}), html: contentClone01041.innerHTML, text: contentClone01041.textContent || '' },
      meta: {
        ...(node.meta || {}),
        dom: { ...domMeta, dataset: commerceBindingDatasetFromElement01067(root), attributes: authoredAttributesFromElement01024(root) },
        commerceComponent01041: true,
        commerceComponentType01041: componentType,
        lastCommerceContentCommit01041: Date.now()
      }
    });
    store.markTransactionChanges([nodeId]);
    const tx = store.commitTransaction({ reason, nodeId, componentType, oneCommit: true });
    persistStore(reason);
    try { window.ST_SAVE_ROOT_DOM_HTML?.({ draft: false, reason, preserveLiveMain: true }); } catch {}
    log('commerce-component-content-committed-01041', { nodeId, componentType, reason, transactionId: tx.id, jsonPrimary: true, oneCommit: true, rootStyleCommitted01046: true, rootStyleCount01046: Object.keys(rootStyle01046).length });
    // 01069: generic event for runtime projections (Product Grid etc.). The authored
    // Store node remains the single design authority; listeners may rebuild live-only
    // projections without persisting duplicated component nodes.
    try { window.dispatchEvent(new CustomEvent('st:commerce-component-content-committed-01041', { detail: { nodeId, componentType, reason, transactionId: tx.id, stage: '01069' } })); } catch {}
    return Object.freeze({ ok: true, nodeId, componentType, transactionId: tx.id });
  } catch (error) {
    try { store.rollbackTransaction({ reason, message: String(error?.message || error || '') }); } catch {}
    return Object.freeze({ ok: false, reason: String(error?.message || error || 'commerce-component-content-error') });
  }
}
...
function resolveMainTemplateTarget00952() {
  const selected = window.__ST_SITE_FRAME_EXPLICIT_SELECTION_00887__?.getActive?.('main') || null;
  const candidates01000 = [];
  const pushCandidate01000 = (value) => {
    const id = String(value || '').trim();
    if (id && !candidates01000.includes(id)) candidates01000.push(id);
  };

  // 01000: the Design sidebar/gallery may take focus after the user selected a Main
  // node. The visual selection can survive while the WeakMap active handle is no
  // longer readable, so preserve the same explicit selection identity through all
  // three already-existing selection representations. Every candidate is validated
  // against the canonical SiteFrameStore before it can become a Replace target.
  pushCandidate01000(selected?.nodeId);
  pushCandidate01000(window.__ST_SITE_FRAME_MAIN_ACTIVE_00887?.nodeId);
  const mainSlot01000 = document.getElementById('st-site-main-slot');
  const liveSelected01000 = mainSlot01000?.querySelector?.('.sf-selection-current.is-selected,.sf-selection-current,[data-sf-id].is-selected,[data-node-id].is-selected') || null;
  if (liveSelected01000 instanceof HTMLElement) {
    pushCandidate01000(liveSelected01000.dataset?.nodeId || liveSelected01000.dataset?.hbRef || liveSelected01000.dataset?.sfId || liveSelected01000.id);
  }

  for (const candidate01000 of candidates01000) {
    const resolved01000 = resolveImmediateMainTemplateTarget00952(store, candidate01000);
    if (resolved01000?.sectionId) return Object.freeze({ ...resolved01000, source: resolved01000.source || 'explicit-selection-01000' });
  }
  return resolveImmediateMainTemplateTarget00952(store, '');
}
...
function commitMainSpecialEffectsDataset01019(reason = 'special-effects') {
  if (store.hasActiveTransaction()) {
    log('main-special-effects-dataset-skipped-01019', { reason, activeTransaction: true }, 'warn');
    return Object.freeze({ ok: false, reason: 'active-transaction', changedIds: [] });
  }

  const main = document.getElementById('st-site-main-slot');
  if (!(main instanceof HTMLElement)) {
    return Object.freeze({ ok: false, reason: 'main-slot-missing', changedIds: [] });
  }
  ...
  const syncDatasetKey = (dataset, key, attrValue, hadStoredValue) => {
    if (attrValue !== null) {
      if (String(dataset[key] ?? '') !== String(attrValue)) {
        dataset[key] = String(attrValue);
        return true;
      }
      return false;
    }
  ...
  main.querySelectorAll('[data-sf-id]').forEach((element) => {
    ...
    const previousDataset = node.meta?.dom?.dataset && typeof node.meta.dom.dataset === 'object'
      ? node.meta.dom.dataset
      : {};
    const dataset = { ...previousDataset };
    let changed = false;

    const sliderAttr = element.getAttribute('data-st-fx-bg-slider');
    const hadSlider = Object.prototype.hasOwnProperty.call(previousDataset, 'stFxBgSlider');
    if (sliderAttr !== null) sliderHosts += 1;
    changed = syncDatasetKey(dataset, 'stFxBgSlider', sliderAttr, hadSlider) || changed;

    const bindAttr = element.getAttribute('data-st-fx-bind-slide');
    const hadBind = Object.prototype.hasOwnProperty.call(previousDataset, 'stFxBindSlide');
    if (bindAttr !== null) boundNodes += 1;
    changed = syncDatasetKey(dataset, 'stFxBindSlide', bindAttr, hadBind) || changed;
  ...
  if (!changedIds.length) {
    log('main-special-effects-dataset-committed-01019', {
      reason,
      sliderHosts,
      boundNodes,
      changedIds: [],
    ...
    return Object.freeze({ ok: true, reason, changedIds: [], sliderHosts, boundNodes, noOp: true });
  }

  const transaction = store.beginTransaction('main-special-effects', { reason, version: '01019' });
  try {
    store.updateNodes(patches, { reason: 'main-special-effects-dataset-01019', emit: false });
    store.markTransactionChanges(changedIds);
    store.commitTransaction({ reason, sliderHosts, boundNodes, jsonPrimary: true });
    persistStore('main-special-effects-dataset-01019');
    log('main-special-effects-dataset-committed-01019', {
      reason,
      transactionId: transaction.id,
      sliderHosts,
      ...
      changedIds,
      jsonPrimary: true,
      rootRenderDeferred: true,
    });
    return Object.freeze({ ok: true, reason, changedIds: Object.freeze([...changedIds]), sliderHosts, boundNodes, transactionId: transaction.id });
  } catch (error) {
    if (store.hasActiveTransaction()) {
      try { store.rollbackTransaction({ message: String(error?.message || error || '') }); } catch {}
    }
    log('main-special-effects-dataset-error-01019', { reason, message: String(error?.message || error || '') }, 'error');
    return Object.freeze({ ok: false, reason: String(error?.message || error || ''), changedIds: [] });
  }
}
...
function updateMainTextContent00903(element, reason = 'main-text-content-00904') {
  ...
  const commerceType01041 = String(owner.dataset.commerceComponent || commerceComponentTypeFromNode01041(node) || '').trim().toLowerCase();
  if (commerceType01041) {
    const clone01041 = owner.cloneNode(true);
    clone01041.querySelectorAll('.st-resize,.st-selection-ui,.st-drag-marker,.st-drop-marker,.hb-resize-handle,.st-resize-handle,.st-section-handle,.st-block-handle,.st-drag-handle,.st-col-resizer,.st-sec-resizer,[data-st-fill-visual-layer="filter"],.st-fill-filter-layer').forEach((artifact) => artifact.remove());
    const changedIds01041 = store.updateNodes({
      [id]: {
        content: { ...(node.content || {}), html: clone01041.innerHTML, text: clone01041.textContent || '' },
        meta: {
          ...(node.meta || {}),
          dom: { ...(node.meta?.dom || {}), dataset: authoredDatasetFromElement00987(owner), attributes: authoredAttributesFromElement01024(owner) },
          textEditable: '01041-commerce-component-content-json-primary',
          commerceComponent01041: true,
          commerceComponentType01041: commerceType01041
        }
      }
    }, { emit: false, rebuildTreeMeta: false });
    if (changedIds01041.length) {
      persistStore(reason);
      try { window.ST_SAVE_ROOT_DOM_HTML?.({ draft: false, reason, preserveLiveMain: true }); } catch {}
    }
    const result01041 = Object.freeze({ ok: true, id, reason, changedIds: changedIds01041, area: 'main', jsonPrimary: true, commerceComponent01041: true, preserveLiveMain: true, textFocusStable: true });
    log('commerce-component-text-content-updated-01041', result01041);
    return result01041;
  }
...
function commitMainTypography00921(element, reason = 'main-typography-controls-00921') {
  ...
  const commerceType01041 = String(owner.dataset.commerceComponent || commerceComponentTypeFromNode01041(node) || '').trim().toLowerCase();
  if (commerceType01041) {
    const committed01041 = commitMainComponentContent01041(owner, reason);
    return Object.freeze({ ...committed01041, id, area: 'main', commerceComponent01041: true, textTargetRole01041: String(editable.dataset?.commerceRole || '') });
  }
...
function fillDatasetFromElement00907(element) {
  ...
  const keys = [
    ...
    'stFillGalleryUrl','stFillGalleryPath','stFillGalleryName','stFillGallerySource','stFillCustomLayer'
  ];
...
function collectMainFillPatches00909(targets = []) {
  ...
  for (const raw of targets || []) {
    ...
    const stylePriority00999 = stylePriorityObjectFromElement00987(styled);
    ...
    if (styled.classList?.contains('st-element-visualfx')) fillClassList.add('st-element-visualfx');
    if (styled.classList?.contains('st-block-surfacefx')) fillClassList.add('st-block-surfacefx');
    ...
    patches[id] = {
      ...
      meta: {
        ...
        fill: {
          ...
          dataset,
          stylePriority: stylePriority00999,
          authority: '00999-single-source-live-layers',
...
function installMainFillPersistence00909() {
  ...
  log('main-fill-live-raf-quiet-installed-00909', {
    ...
    continuousRangesDomPreviewOnly00997: true,
    oneFinalCommitPerGesture00997: true,
    blockSurfacePreparedOncePerGesture00997: true,
    quietLiveLogs: true,
    finalCommit: true,
    selectionLossFlush: true,
...
let historyPersistEpoch01000 = 0;
function persistHistoryAfterCanonicalStore01000(reason = 'transaction:commit') {
  const epoch01000 = ++historyPersistEpoch01000;
  queueMicrotask(() => {
    if (epoch01000 !== historyPersistEpoch01000) return;
    persistHistory(reason);
  });
}
...
store.subscribe((event) => {
  if (event?.type === 'transaction:commit') {
    // 01000 storage priority contract: SiteFrameStore JSON is the source of truth.
    // The completed action writes STORAGE_KEY synchronously after commit; history is
    // secondary persistence and therefore runs in the following microtask. This
    // prevents a large history snapshot from consuming quota before the canonical
    // Store can be saved. Runtime Undo/Redo is still recorded synchronously in memory.
    persistHistoryAfterCanonicalStore01000(`transaction:${event?.detail?.label || 'commit'}`);
    updateHistoryButtons00899();
  } else if (event?.type === 'history:undo' || event?.type === 'history:redo' || event?.type === 'history:clear') {
    persistHistoryAfterCanonicalStore01000(event.type);
    updateHistoryButtons00899();
  }
});


const ELEMENT_INFO_SAFE_STYLE_KEY_01008 = /^(?:--[a-zA-Z0-9_-]+|[-a-zA-Z][a-zA-Z0-9-]*)$/;
const ELEMENT_INFO_READONLY_STYLE_KEYS_01008 = new Set([
  '--st-block-surface-source-b64',
  '--st-fill-authority-v',
]);

function commitElementInfoStyle01008(input = {}) {
  const nodeId = String(input?.nodeId || '').trim();
  const property = String(input?.property || '').trim();
  const value = String(input?.value ?? '');
  const reason = String(input?.reason || 'element-info-style-01008');
  if (!nodeId || !property || !ELEMENT_INFO_SAFE_STYLE_KEY_01008.test(property)) {
    return Object.freeze({ ok: false, reason: 'invalid-element-info-style-request' });
  }
  if (property.startsWith('--sf-') || ELEMENT_INFO_READONLY_STYLE_KEYS_01008.has(property)) {
    return Object.freeze({ ok: false, reason: 'readonly-system-style-property', property });
  }
  if (store.hasActiveTransaction()) {
    return Object.freeze({ ok: false, reason: 'active-site-frame-transaction' });
  }
  const node = store.maybeGet(nodeId);
  if (!node || node.kind === KIND.SITE || node.kind === KIND.AREA) {
    return Object.freeze({ ok: false, reason: 'element-info-node-not-editable', nodeId });
  }

  const nextStyle = { ...(node.style || {}), [property]: value };
  const patch = {
    style: nextStyle,
    meta: {
      ...(node.meta || {}),
      elementInfo01008: {
        ...(node.meta?.elementInfo01008 || {}),
  ...
  store.beginTransaction('element-info-style', {
    nodeId,
    property,
    reason,
    source: 'element-info-01008',
  });
  try {
    const scope = getResponsiveEditScope00991();
    const scoped = scopeVisualPatches00991({ [nodeId]: patch }, scope);
    const changedIds = store.updateNodes(scoped, { emit: false, rebuildTreeMeta: false });
    store.markTransactionChanges(changedIds);
    const rendered = renderer.renderNodes(changedIds);
    const result = store.commitTransaction({
      reason,
      nodeId,
      property,
      value,
      source: 'element-info-01008',
      responsiveProfileId: scope?.scoped ? scope.profileId : 'base',
      oneCommit: true,
    });
    // Canonical Store is persisted synchronously. History persistence is secondary
    // and remains deferred by the 01000 storage-priority subscriber.
    persistStore(reason);
    try { window.ST_SAVE_ROOT_DOM_HTML?.({ draft: false, reason, preserveLiveMain: node.area === 'main' }); } catch {}
    try {
      window.dispatchEvent(new CustomEvent('st:element-info-property-committed-01008', {
        detail: { nodeId, property, value, changedIds, rendered, transactionId: result.id }
      }));
    } catch {}
    log('element-info-style-committed-01008', {
      nodeId, property, value, changedIds, rendered, transactionId: result.id,
      responsiveProfileId: scope?.scoped ? scope.profileId : 'base',
      storeAuthority: true,
      oneCommit: true,
    });
    return Object.freeze({ ok: true, nodeId, property, value, changedIds, rendered, transactionId: result.id });
  } catch (error) {
    try { store.rollbackTransaction({ message: String(error?.message || error || '') }); } catch {}
    log('element-info-style-commit-error-01008', {
      nodeId, property, message: String(error?.message || error || '')
    }, 'error');
    return Object.freeze({ ok: false, reason: String(error?.message || error || '') });
  }
}

function renderElementInfoNode01008(nodeId) {
  const id = String(nodeId || '').trim();
  if (!id || !store.maybeGet(id)) return false;
  return renderer.renderNodes([id]) > 0;
}
...
const api = Object.freeze({
  ...
  contract: Object.freeze({
    ...
    transactions: true,
    mainTemplateReplaceMainAreaWithoutSectionTarget01000: true,
    mainTemplateLastExplicitSelectionTarget01000: true,
    canonicalStoreBeforePersistedHistory01000: true,
    observers: 0,
    timers: 0,
    retryLoops: 0,
    ...
    mainResize: true,
    mainTemplates: true,
    commerceComponentTemplates01041: true,
    commerceComponentStoreOwned01041: true,
    commerceComponentDirectDomInsert01041: false,
    ...
    elementInfoStyleAuthority01008: true,
    elementInfoDirectEditOneCommit01008: true,
    mainFillStyleJsonPrimary: true,
    ...
    mainFillBackgroundAndBlockSurfaceIndependent00998: true,
    mainFillBackgroundOpacityOwnsExplicitFillOnly00998: true,
    mainFillBlockTransparencyOwnsAuthoredSurfaceOnly00998: true,
    mainFillSingleAuthority00999: true,
    mainFillPhysicalSurfaceFillFilterLayers00999: true,
    mainFillFilterOpacityAuthority00999: '--st-bgfx-filter-opacity',
    mainFillBlockBlurAuthority00999: '--st-block-surface-blur',
    mainFillAllRangeControlsRafSingleCommit00999: true,
  ...
  commitMainSpecialEffectsDataset01019,
  commitElementInfoStyle01008,
  renderElementInfoNode01008,
  ...
  exportMainAreaSnapshot01040,
  restoreMainAreaSnapshot01040,
  emptyMainAreaSnapshot01040,
  ensureMainSelectionFrame,
  renderMainSelectionArea,
  applyMainTemplate: applyMainTemplate00888,
  resolveMainComponentTarget01041,
  applyMainComponentTemplate01041,
  commitMainComponentContent01041,
  commitMainComponentBindingDataset01067,
...
function boot() {
  ...
  let levelLayoutMigration01025 = null;
  try { levelLayoutMigration01025 = migrateImportedMainLevelLayout01025(); }
  catch (error) { log('main-level-layout-migration-error-01025', { message: String(error?.message || error || '') }, 'error'); }
  try {
    restoreCommittedState('boot');
    if (levelLayoutMigration01025?.changed > 0) persistStore('01025-main-level-layout-migration');
  }
...
window.addEventListener('st:canvas-apply-snapshot', (event) => {
  ...
  try {
    if (snapshot && typeof snapshot === 'object') {
      const scoped = snapshot.siteFrameMain01040;
      if (scoped && typeof scoped === 'object') {
        restoreMainAreaSnapshot01040(scoped, { reason: 'canvas-snapshot:page-main-01040', persist: true });
        return;
      }
      const hasPageIdentity = !!String(snapshot?.page?.id || snapshot?.site?.page?.id || '').trim();
      const hasHtml = !!String(snapshot.pageHTML || snapshot.rootHTML || snapshot.previewHtml || '').trim();
      if (hasPageIdentity && !hasHtml) {
        restoreMainAreaSnapshot01040(emptyMainAreaSnapshot01040(), { reason: 'canvas-snapshot:empty-page-main-01040', persist: true });
        return;
      }
      // Legacy per-page snapshots with HTML are imported by workspace runtime
      // after that HTML is mounted. Do not overwrite them with the last global Main.
      if (hasPageIdentity && hasHtml) return;
    }
    restoreCommittedState('canvas-snapshot');
  } catch (error) {
    log('page-main-snapshot-restore-error-01040', { message: String(error?.message || error || '') }, 'error');
  }
}, true);
