...
function saveRootDom(options = {}) {
  ...
  const payload = snapshotPayload(root?.innerHTML || '', options.reason || 'explicit-root-save-00914');
...
function applySnapshot(snapshot, options = {}) {
  ...
  log('snapshot-applied', { pageId, htmlLength: html.length, bundle: snapshot.__st_bundle_v1 === true, mainSelectionNodes: frame.main.querySelectorAll('[data-main-selection-stage="00914"]').length });
  return true;
}
