...
export function initPageTreePanel() {
  ...
  // 01021: exact Site <-> Tree focus synchronization is opt-in. The Tree becomes
  // the active inspector target when the user opens/clicks it, and loses that
  // status when another Design accordion is activated. Bulk top-selector modes
  // may mirror colors, but never move exact focus.
  const treeSection01021 = treeRoot.closest?.('.design-section--page-tree') || null;
  function isTreeAccordionOpen01021_() {
    return !!treeSection01021?.classList?.contains('is-open');
  }
  function isTreeFocusActive01021_() {
    return isTreeAccordionOpen01021_() && treeSection01021?.dataset?.treeFocusActive01021 === '1';
  }
  function setTreeFocusActive01021_(on) {
    if (!treeSection01021) return;
    treeSection01021.dataset.treeFocusActive01021 = on ? '1' : '0';
    treeSection01021.classList.toggle('is-tree-focus-active-01021', !!on);
    if (!on) {
      treeRoot.querySelectorAll('.page-tree-row--active, .page-tree-block--active, .page-tree-special--active, .page-tree-mode-match-01020').forEach((node) => {
        node.classList.remove('page-tree-row--active', 'page-tree-block--active', 'page-tree-special--active', 'page-tree-mode-match-01020');
      });
    }
  }
  setTreeFocusActive01021_(false);

  treeSection01021?.addEventListener?.('pointerdown', (ev) => {
    if (ev.target instanceof Element && ev.target.closest('.design-section--page-tree')) {
      if (isTreeAccordionOpen01021_()) setTreeFocusActive01021_(true);
    }
  }, true);
  document.getElementById('design-panel-root')?.addEventListener?.('click', (ev) => {
    const header = ev.target instanceof Element ? ev.target.closest('.design-section__header') : null;
    if (!header) return;
    const owner = header.closest('.design-section');
    if (owner && owner !== treeSection01021) setTreeFocusActive01021_(false);
  }, true);
  window.addEventListener('st:pageTreeFocusActive01021', (ev) => {
    const active = !!ev?.detail?.active;
    setTreeFocusActive01021_(active);
    if (active) {
      renderTree();
      applyTreeModeHighlights01020_();
    }
  });
  ...
  // 01020: the top Design selector (Sections / Levels / Containers / Blocks /
  // Headings / Text / Icons) and the Page Tree share the same visual palette.
  const TREE_MODE_ORDER_01020 = ['sections', 'levels', 'containers', 'blocks', 'headings', 'texts', 'icons'];
  const activeDesignModes01020 = new Set(
    TREE_MODE_ORDER_01020.filter((mode) => document.documentElement.classList.contains(`st-design-select-${mode}`))
  );
  ...
  function revealAndScrollTreeNode01021_(node) {
    if (!(node instanceof HTMLElement) || !isTreeFocusActive01021_()) return;
    // Expand every collapsed parent branch so a canvas click always produces a
    // visible row in the tree, not a highlight hidden inside a closed subtree.
    let ul = node.parentElement?.closest?.('ul');
    while (ul && treeRoot.contains(ul)) {
      const ownerLi = ul.parentElement;
      const ownerHead = ownerLi?.firstElementChild;
      if (ownerHead?.__stCollapseKey) {
        setCollapsed(ownerHead.__stCollapseKey, false);
        try { ownerHead.__stApplyCollapse?.(); } catch (_) {}
      }
      ul = ownerLi?.parentElement?.closest?.('ul') || null;
    }
    try { node.scrollIntoView({ block: 'nearest', inline: 'nearest' }); } catch (_) {}
    try { node.setAttribute('tabindex', '-1'); node.focus({ preventScroll: true }); } catch (_) {}
  }

  function resolveSelectionElements01020_(sel) {
    const direct = Array.isArray(sel?.elements) ? sel.elements.filter((el) => el instanceof HTMLElement) : [];
    if (direct.length) return direct;
    const one = sel?.element instanceof HTMLElement ? sel.element : (sel?.el instanceof HTMLElement ? sel.el : null);
    if (one) return [one];
    const id = String(sel?.id || sel?.nodeId || '').trim();
    if (!id) return [];
    const escaped = cssIdent_(id);
    try {
      const byId = siteRoot.querySelector(
        `[data-node-id="${escaped}"], [data-sf-id="${escaped}"], [data-st-node-id="${escaped}"], [data-hb-ref="${escaped}"], [data-uid="${escaped}"]`
      );
      return byId instanceof HTMLElement ? [byId] : [];
    } catch (_) {
      return [];
    }
  }

  function markTreeModeKind01020_(node, kind) {
    if (!(node instanceof HTMLElement)) return node;
    const k = String(kind || '').trim();
    if (k) node.dataset.treeModeKind = k;
    return node;
  }

  function applyTreeModeHighlights01020_() {
    treeRoot.querySelectorAll('.page-tree-mode-match-01020').forEach((node) => node.classList.remove('page-tree-mode-match-01020'));
    if (!activeDesignModes01020.size) return;
    activeDesignModes01020.forEach((mode) => {
      treeRoot.querySelectorAll(`[data-tree-mode-kind="${cssIdent_(mode)}"]`).forEach((node) => {
        node.classList.add('page-tree-mode-match-01020');
      });
    });
  }

  function publishTreeSelection01020_(el) {
    if (!(el instanceof HTMLElement)) return;
    // Keep the existing selection manager as the authority. This restores the
    // old two-way contract: clicking Tree is also a real Builder selection.
    let area = 'main';
    if (el.closest?.('#st-site-header-slot')) area = 'header';
    else if (el.closest?.('#st-site-footer-slot')) area = 'footer';
    else if (el.closest?.('#st-site-main-slot')) area = 'main';
    try { window.ST_SELECTION?.setSingle?.(el, { type: `${area}-inner` }); } catch (_) {}
  }
  ...
  const TREE_CANVAS_CLASSES = [
    'st-tree-selected-section',
    'st-tree-selected-level',
  ...
  function clearTreeCanvasHighlights() {
    const selector = [
      ...
      '.st-tree-selected-section', '.st-tree-selected-level', '.st-tree-selected-container', '.st-tree-selected-block', '.st-tree-selected-icon', '.st-tree-selected-icon-inner', '.st-tree-selected-text', '.st-tree-selected-heading',
    ].join(', ');
  ...
  function getSpecialVisualRole(key, el) {
    ...
    if (k.includes('__sec')) return 'section';
    if (k.includes('__row')) return 'level';
    ...
    if (k === 'header' || k === 'main' || k === 'footer') return 'block';
    return 'block';
  }
  ...
  function applyTreeCanvasHighlight(el, key) {
    ...
    targets.forEach((target) => {
      ...
      if (role === 'section') target.classList.add('st-tree-selected-section');
      else if (role === 'level') target.classList.add('st-tree-selected-level');
      else if (role === 'container') target.classList.add('st-tree-selected-container');
...
function updateCanvasSelection(scrollTargetEl = null) {
  ...
  function selectBlock(blockId, { append = false, scroll = false } = {}) {
    ...
    updateTreeSelection();
    if (!append && blockEl instanceof HTMLElement) publishTreeSelection01020_(blockEl);
  }
  ...
  function selectRow(rowId, { append = false, scroll = false } = {}) {
    ...
    updateTreeSelection();
    if (!append && rowEl instanceof HTMLElement) publishTreeSelection01020_(rowEl);
  }
  ...
  function renderTree() {
    ...
    // після перебудови дерева накладаємо поточну підсвітку + глобальні режими інспектора
    updateTreeSelection();
    applyTreeModeHighlights01020_();
  }
  ...
  function makeSpecialRootNode(specialKey, title, slotEl) {
    ...
    // 01021: Main is not always authored as section -> row -> block. Current
    // Marketplace Main has one outer .st-section and then many nested SiteFrame
    // containers/rows/blocks. Main therefore uses a recursive real-DOM tree,
    // while Header/Footer retain their established section-level contract.
    ...
    if (sections.length) {
      ...
      sections.forEach((secEl, si) => {
        const treeNode = specialKey === 'main' ? makeMainDomNode01021_(secEl, si) : makeSpecialSectionNode(specialKey, secEl, si);
        if (treeNode) childUl.appendChild(treeNode);
      });
    }
  ...
  function makeSpecialSectionNode(specialKey, secEl, si) {
    ...
    markTreeModeKind01020_(head, 'sections');
    head.style.cursor       = 'pointer';
    head.style.padding      = '4px 8px';
    head.style.borderRadius = '8px';
    head.style.fontSize     = '12px';
  ...
  function isMainRuntimeOnlyNode01021_(el) {
    if (!(el instanceof HTMLElement)) return true;
    if (el.matches('.st-resize, .st-drop-marker, .st-fx-layer, .st-fx-slide-layer, .st-selection-overlay, [data-st-runtime-only="1"], script, style')) return true;
    if (el.closest('.st-fx-runtime-layer, [data-st-fx-runtime="1"]')) return true;
    return false;
  }

  function mainTreeRole01021_(el) {
    if (!(el instanceof HTMLElement) || isMainRuntimeOnlyNode01021_(el)) return null;
    if (el.classList.contains('st-section')) return { key:'main__sec', mode:'sections', cls:'page-tree-row', label:'Секція' };
    if (el.classList.contains('st-row')) return { key:'main__row', mode:'levels', cls:'page-tree-row', label:'Рівень' };
    if (el.classList.contains('st-block')) {
      if (!el.classList.contains('hb-elem')) return { key:'main__container', mode:'containers', cls:'page-tree-block page-tree-node--container', label:'Контейнер' };
      const kind = getSpecialElementKind(el);
      if (kind === 'heading') return { key:'main__elem', mode:'headings', cls:'page-tree-block page-tree-node--heading', label:'Заголовок' };
      if (kind === 'icon') return { key:'main__elem', mode:'blocks', cls:'page-tree-block page-tree-node--block page-tree-node--icon-block', label:'Блок Іконка' };
      return { key:'main__elem', mode:'blocks', cls:'page-tree-block page-tree-node--block', label:'Блок' };
    }
    if (el.matches('.st-text-edit, [data-st-text-target="1"], .st-menu__text, .st-button__label, .st-phone__text, .st-logo__title, .st-logo__subtitle')) {
      return { key:'main__text', mode:'texts', cls:'page-tree-block page-tree-node--text', label:'Текст' };
    }
    if (el.matches('.st-icon-btn, .st-button__iconbtn, .st-phone__iconbtn, .st-logo__iconbtn, .st-logo__mark, .st-icon-svg, .st-phone__iconsvg, .st-button__iconsvg, svg[data-st-icon]')) {
      return { key:'main__icon', mode:'icons', cls:'page-tree-block page-tree-node--icon-inner', label:'Іконка' };
    }
    if (el.matches('.st-menu__link, [data-st-menu-item="1"]')) return { key:'main__menuitem', mode:'blocks', cls:'page-tree-block page-tree-node--menuitem', label:'Пункт меню' };
    return null;
  }

  function mainMeaningfulChildren01021_(parent) {
    const out = [];
    const walkDirect = (host) => {
      Array.from(host?.children || []).forEach((child) => {
        if (!(child instanceof HTMLElement) || isMainRuntimeOnlyNode01021_(child)) return;
        const role = mainTreeRole01021_(child);
        if (role) out.push(child);
        else walkDirect(child); // transparent authored wrapper: keep walking until a real editor node is found
      });
    };
    walkDirect(parent);
    return out;
  ...
  function mainTreeLabel01021_(el, role, index = 0) {
    const custom = String(el?.dataset?.stTreeName || el?.dataset?.name || el?.getAttribute?.('aria-label') || '').trim();
    if (custom) return custom;
    if (role?.mode === 'texts') return `Текст: ${textBrief(el, 'Текст')}`;
    if (role?.mode === 'icons') return iconBrief(el, 'Іконка');
    if (role?.mode === 'headings') return `Заголовок: ${textBrief(el.querySelector?.('.st-text-edit') || el, `Заголовок ${index + 1}`)}`;
    if (role?.key === 'main__elem') return defaultElementLabel(el, index);
    const classHint = Array.from(el?.classList || []).find((c) => /^shifttime02-/.test(c) && !/main-template/.test(c));
    if (classHint) return `${role?.label || 'Елемент'} · ${classHint.replace(/^shifttime02-/, '')}`;
    return `${role?.label || 'Елемент'} ${index + 1}`;
  }

  function makeMainDomNode01021_(el, index = 0) {
    const role = mainTreeRole01021_(el);
    if (!role) return null;
    const li = document.createElement('li');
    const node = document.createElement('div');
    node.dataset.specialKey = role.key;
    node.dataset.specialUid = ensureSpecialUid(el, role.key) || '';
    node.className = role.cls;
    markTreeModeKind01020_(node, role.mode);
    node.style.cursor = 'pointer';
    node.style.padding = '4px 8px';
    node.style.borderRadius = '8px';
    node.style.fontSize = '12px';

    const kids = mainMeaningfulChildren01021_(el);
    let childUl = null;
    if (kids.length) {
      childUl = makeUl('4px 0 0 12px', '2px');
      kids.forEach((kid, ki) => {
        const childNode = makeMainDomNode01021_(kid, ki);
    ...
    node.addEventListener('click', (ev) => {
      ev.stopPropagation();
      setTreeFocusActive01021_(true);
      selectSpecial(el, role.key, { scroll: true });
    });
    toggleCollapseOnDblClick(node, collapseKey, childUl);

    decorateHeadWithControls(node, mainTreeLabel01021_(el, role, index), {
      key: childUl ? collapseKey : null,
      childUl,
      editable: true,
      deleteTarget: { kind:'special', el, specialKey:role.key },
      rename: {
        kind:'special', el,
        get: () => String(el?.dataset?.stTreeName || '').trim() || mainTreeLabel01021_(el, role, index),
        set: (v) => setSpecialTreeName_(el, role.key, v),
      },
    });
    li.appendChild(node);
    if (childUl) li.appendChild(childUl);
  ...
  function makeDomLeafNode(specialKey, item) {
    ...
    if (role === 'text') markTreeModeKind01020_(node, 'texts');
    else if (role === 'icon-inner' || role === 'icon') markTreeModeKind01020_(node, 'icons');
    node.style.cursor = 'pointer';
    node.style.padding = '4px 8px';
    node.style.borderRadius = '8px';
    node.style.fontSize = '12px';
  ...
  function makeSpecialElementNode(specialKey, el, ei) {
    ...
    markTreeModeKind01020_(node, specialElementKind === 'heading' ? 'headings' : 'blocks');
  ...
  function makeSpecialLevelNode(specialKey, rowEl, ri) {
    ...
    const collapseKey = makeCollapseKey('special', head.dataset.specialUid);
    head.className = 'page-tree-row';
    markTreeModeKind01020_(head, 'levels');
    head.style.cursor       = 'pointer';
    head.style.padding      = '4px 8px';
    head.style.borderRadius = '8px';
    head.style.fontSize     = '12px';
    ...
    if (containers.length) {
      ...
      containers.forEach((contEl, ci) => {
        ...
        markTreeModeKind01020_(contHead, 'containers');
 ...
 function makeBlockNode(blockId, index, prefix) {
  ...
  markTreeModeKind01020_(line, b?.childrenRow ? 'containers' : 'blocks');
  ...
  document.addEventListener('st:selection-changed', (ev) => {
    ...
    // 01021: a canvas click mirrors exact focus into Tree only while the Tree
    // accordion is open AND has been activated by the user. Closed/inactive Tree
    // remains untouched. Bulk Design modes are handled separately below.
    if (!isTreeFocusActive01021_()) return;
    if (!sel) {
      resetSelection();
      updateTreeSelection();
      applyTreeModeHighlights01020_();
    ...
    const els = resolveSelectionElements01020_(sel);
    const type = sel.type || null;

    // 01021: Global Design modes are a bulk selection. They only add category
    // highlights and MUST NOT move or clear the current exact focus.
    if (type === 'design-mode-multi') {
      applyTreeModeHighlights01020_();
      return;
    }

    // Exact single selection may replace the current exact focus.
    resetSelection();

    // ---- CANVAS ----
    if (type === 'block') {
      const mainEl = els.find((el) => el instanceof HTMLElement && !!el.closest?.('#st-site-main-slot')) || null;
      if (mainEl) {
        const role = mainTreeRole01021_(mainEl) || mainTreeRole01021_(mainEl.closest?.('.st-block'));
        const targetEl = mainTreeRole01021_(mainEl) ? mainEl : mainEl.closest?.('.st-block');
        if (role && targetEl) {
          specialSelectedEl = targetEl;
          specialSelectedKey = role.key;
          specialSelectedUid = ensureSpecialUid(targetEl, role.key);
          updateTreeSelection();
          revealAndScrollTreeNode01021_(findSpecialTreeNode_(specialSelectedUid, specialSelectedKey));
          return;
        }
      }
      selectionMode = 'block';
      ...
      revealAndScrollTreeNode01021_(node);
      return;
    }
  ...
  // 01020: top inspector modes are mirrored in Tree using the very same
  // category palette already used on the canvas. Multiple modes can be active.
  window.addEventListener('st:designSelectionModeChange', (ev) => {
    if (!isTreeFocusActive01021_()) return;
    const modes = Array.isArray(ev?.detail?.modes) ? ev.detail.modes : [];
    activeDesignModes01020.clear();
    modes.forEach((mode) => {
      const m = String(mode || '').trim();
      if (TREE_MODE_ORDER_01020.includes(m)) activeDesignModes01020.add(m);
    });
    applyTreeModeHighlights01020_();
  });
  ...
  // 01021 diagnostics for ALL LOG / manual verification.
  try {
    window.ST_PAGE_TREE_01021 = Object.freeze({
      version: '01021-page-tree-full-main-focus-scroll-parity',
      isOpen: () => isTreeAccordionOpen01021_(),
      isFocusActive: () => isTreeFocusActive01021_(),
      nodeCount: () => treeRoot.querySelectorAll('[data-special-uid], [data-row-id], [data-block-id]').length,
      mainNodeCount: () => treeRoot.querySelectorAll('[data-special-key^="main"]').length,
      scroll: () => ({ clientHeight: wrap.clientHeight, scrollHeight: wrap.scrollHeight, maxScroll: Math.max(0, wrap.scrollHeight - wrap.clientHeight) }),
    });
  } catch (_) {}
  ...
  const mo = new MutationObserver(() => {
    if (!isTreeAccordionOpen01021_()) return;
    renderTree();
  });
