...
export async function initMarketplaceStudio01073(){
  ...
  if(studio){studio.dataset.mpStage='01073';const metrics=studio.querySelector('[data-mp-system-status-block]')?.querySelectorAll('.mp-context__metric')||[];if(metrics[0])metrics[0].innerHTML='<span>Studio stage</span><b>01073</b>';if(metrics[1])metrics[1].innerHTML='<span>Global catalog</span><b>SEARCH INDEX</b>';if(metrics[2])metrics[2].innerHTML='<span>Catalog model</span><b>PRODUCT + OFFERS</b>';if(metrics[3])metrics[3].innerHTML='<span>DEMO fixture</span><b>5 PRODUCTS / 2 CATS</b>';const warn=studio.querySelector('.mp-context__notice.is-warn');if(warn)warn.innerHTML='<b>Global Catalog 01073:</b> центральний пошук читає тільки approved MarketplaceListing/SellerOffer. Один CatalogProduct може мати кілька пропозицій продавців. У чистому LOCAL Store автоматично додається DEMO-набір 5 товарів у 2 категоріях для тестування.';}
