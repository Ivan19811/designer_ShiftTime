...
async function request(path,{method='GET',body,token=state.token}={}){const controller=new AbortController(),timeout=getMarketplaceBackendConfig01071().requestTimeoutMs||12000,t=setTimeout(()=>controller.abort(),timeout);try{const headers={'accept':'application/json'};if(body!==undefined)headers['content-type']='application/json';if(token)headers.authorization=`Bearer ${token}`;const res=await fetch(`${baseUrl()}${path}`,{method,headers,body:body===undefined?undefined:JSON.stringify(body),signal:controller.signal});const text=await res.text();let data={};try{data=text?JSON.parse(text):{};}catch{data={error:text||`HTTP ${res.status}`};}if(!res.ok)throw Object.assign(new Error(data.error||`HTTP ${res.status}`),{statusCode:res.status,data});return data;}finally{clearTimeout(t);}}
function acceptSession(payload,reason){const token=String(payload?.token||state.token||''),user=payload?.user||null,scope=payload?.scope||null,expiresAt=payload?.expiresAt||state.expiresAt||null;if(token)persistToken(token);return setState({status:user?'authenticated':'anonymous',token,user,scope,expiresAt,lastError:''},reason);}
function clearSession(reason,lastError=''){persistToken('');return setState({status:'anonymous',token:'',user:null,scope:null,expiresAt:null,lastError},reason);}
