...
function desired(){const c=getMarketplaceBackendConfig01071(),s=getMarketplaceBackendStatus01071();return c.mode==='api'&&s.state==='api'?new ApiMarketplacePaymentRepository01076({baseUrl:c.apiBaseUrl,requestTimeoutMs:c.requestTimeoutMs}):new LocalMarketplacePaymentRepository01076();}
...
export async function initMarketplacePaymentRuntime01076(){const s=getMarketplacePaymentStore01076();await s.init();if(!installed){installed=true;window.addEventListener('st:marketplace-backend-status-changed',()=>{const n=desired();if(n.type!==s.getRepositoryInfo().type)s.setRepository(n).catch(()=>{});});window.addEventListener('st:marketplace-tenant-context-changed',()=>s.refresh().catch(()=>{}));window.addEventListener('st:marketplace-orders-changed',()=>s.refresh().catch(()=>{}));}try{window.__ST_ALL_LOG__?.push?.('marketplace-payments:runtime-ready-01079',{repository:s.getRepositoryInfo(),commissionFoundation:true,payoutLedger:true,realProvider:false,pricingStage:'01079'});}catch{}return s;}
