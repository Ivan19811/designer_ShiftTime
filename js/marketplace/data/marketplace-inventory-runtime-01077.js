...
function desired(){const c=getMarketplaceBackendConfig01071(),s=getMarketplaceBackendStatus01071();return s.state==='api'?new ApiMarketplaceInventoryRepository01077({baseUrl:c.apiBaseUrl,requestTimeoutMs:c.requestTimeoutMs}):new LocalMarketplaceInventoryRepository01077();}
...
export async function initMarketplaceInventoryRuntime01077(){const s=getMarketplaceInventoryStore01077();await s.init();if(!installed){installed=true;window.addEventListener('st:marketplace-backend-status-changed',()=>{const n=desired();syncMarketplaceOperationalRepository01089(s,n,{reason:'auth/backend-context-changed',refresh:()=>s.refresh()}).catch(()=>{});});window.addEventListener('st:marketplace-tenant-context-changed',()=>s.refresh().catch(()=>{}));window.addEventListener('st:marketplace-orders-changed',()=>s.refresh().catch(()=>{}));window.addEventListener('st:marketplace-payments-changed',()=>s.refresh().catch(()=>{}));window.addEventListener('st:marketplace-network-changed',()=>s.refresh().catch(()=>{}));}try{window.__ST_ALL_LOG__?.push?.('marketplace-inventory:runtime-ready-01077',{repository:s.getRepositoryInfo(),reservation:true,stockCommit:true,backendAuthoritative:true});}catch{}return s;}
