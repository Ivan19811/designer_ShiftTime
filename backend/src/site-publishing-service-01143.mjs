...
function mapRow(r){if(!r)return null;return {
  ...
  lastPublishedAt:r.last_published_at?new Date(r.last_published_at).toISOString():null,trafficIdentityToken:r.traffic_identity_token||'',
};}
...
export function createPostgresPublishingRepository01143(){
  return {
    ...
    async ensureTrafficIdentity(scope,builderSiteId,token){return withClient(async c=>{const q=await c.query(`UPDATE shifttime_published_sites SET traffic_identity_token=CASE WHEN COALESCE(traffic_identity_token,'')='' THEN $1 ELSE traffic_identity_token END,updated_at=now() WHERE account_id=$2 AND workspace_id=$3 AND store_id=$4 AND builder_site_id=$5 RETURNING *`,[token,scope.accountId,scope.workspaceId,scope.storeId,builderSiteId]);if(!q.rowCount)throw Object.assign(new Error('Published site identity not found'),{statusCode:404});return mapRow(q.rows[0]);});},
