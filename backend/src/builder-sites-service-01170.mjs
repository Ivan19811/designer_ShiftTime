...
export function createBuilderSitesRepository01170(){
  ...
  return {
    ...
    async saveMetadata(scope,id,input){const data=normalizeBuilderSiteInput01170(input,id);return withClient(async c=>{
      const q=await c.query(`UPDATE shifttime_builder_sites SET name=$1,slug=$2,description=$3,updated_at=now() WHERE account_id=$4 AND workspace_id=$5 AND store_id=$6 AND id=$7 RETURNING id`,[data.name,data.slug,data.description,scope.accountId,scope.workspaceId,scope.storeId,data.id]);
      if(!q.rowCount)return null;
      const row=await c.query(`${select} WHERE s.account_id=$1 AND s.workspace_id=$2 AND s.store_id=$3 AND s.id=$4 LIMIT 1`,[scope.accountId,scope.workspaceId,scope.storeId,data.id]);
...
const defaultRepo=createBuilderSitesRepository01170();
const defaultService=createBuilderSitesService01170({repo:defaultRepo,netlify:createNetlifyClient01143({token:config.netlifyAuthToken,baseUrl:config.netlifyApiBaseUrl})});
...
export const saveBuilderSiteMetadata01231=async(scope,id,input)=>{const row=await defaultRepo.saveMetadata(scope,clean(id),input);if(!row)throw httpError('Builder site not found',404,'ST_BUILDER_SITE_NOT_FOUND');return row;};
