...
export function buildIntegrationTrafficEvent01201(input={}){
  ...
  return Object.freeze({
    ...
    inboundBytes:bytes(input.inboundBytes),outboundBytes:bytes(input.outboundBytes),renderBillableOutboundBytes:bytes(input.outboundBytes),statusCode,result:failed?'failed':'success',durationMs:Math.max(0,Math.round(finishedAtMs-startedAtMs)),
  });
}
