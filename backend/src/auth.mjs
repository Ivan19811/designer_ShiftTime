...
export async function authenticateRequest(req){const token=bearer(req);if(!token){const e=new Error('Authorization Bearer token is required');e.statusCode=401;throw e;}const tokenHash=hashToken(token);return withClient(async client=>{const q=await client.query(`SELECT s.id session_id,s.user_id,u.email,u.name FROM api_sessions s JOIN platform_users u ON u.id=s.user_id WHERE s.token_hash=$1 AND s.status='active' AND u.status='active' AND (s.expires_at IS NULL OR s.expires_at>now()) LIMIT 1`,[tokenHash]);if(!q.rowCount){const e=new Error('Invalid or expired session');e.statusCode=401;throw e;}await client.query('UPDATE api_sessions SET last_seen_at=now() WHERE id=$1',[q.rows[0].session_id]);return {sessionId:q.rows[0].session_id,userId:q.rows[0].user_id,email:q.rows[0].email,name:q.rows[0].name};});}
