...
export async function loadAuthorizedStorageReferenceState01210(scope={},options={}){
  ...
  const [mediaRows,derivativeRows,siteRows,marketRows,tableRows]=await Promise.all([
    safeRows01210(query,`SELECT id,account_id,workspace_id,store_id,provider,bucket,object_key,kind,file_name,mime_type,size_bytes,status,metadata,updated_at,completed_at,deleted_at FROM media_cloud_assets WHERE account_id=$1 AND status<>'deleted'`,[accountId]),
    safeRows01210(query,`SELECT d.id,d.media_asset_id,d.derivative_kind,d.width,d.height,d.mime_type,d.object_key,d.status,d.metadata,d.updated_at,a.account_id,a.workspace_id,a.store_id,a.provider,a.bucket,a.file_name AS source_file_name FROM media_asset_derivatives d JOIN media_cloud_assets a ON a.id=d.media_asset_id WHERE a.account_id=$1 AND a.status<>'deleted'`,[accountId]),
