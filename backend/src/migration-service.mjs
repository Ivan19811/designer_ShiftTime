...
const JOURNAL_SQL=`CREATE TABLE IF NOT EXISTS shifttime_schema_migrations(
  ...
  applied_stage text NOT NULL DEFAULT '01092',
...
export async function applyMigrations01080({dryRun=false,logger=console}={}){
  const client=await pool.connect();try{
    ...
    for(const m of plan){if(m.action==='skip'){logger.log?.(`[01092] skip ${m.filename}`);continue;}const started=Date.now();logger.log?.(`[01092] migrate ${m.filename}`);await client.query(m.sql);const ms=Date.now()-started;await client.query('INSERT INTO shifttime_schema_migrations(filename,checksum_sha256,applied_stage,execution_ms) VALUES($1,$2,$3,$4)',[m.filename,m.checksum,'01092',ms]);}
    try{await client.query(`UPDATE platform_deployment_state SET schema_stage='01092',last_migration_at=now(),updated_at=now() WHERE id='primary'`);}catch{}
