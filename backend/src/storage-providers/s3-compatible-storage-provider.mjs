...
export class S3CompatibleStorageProvider01081{
  ...
  async putObject({key,body,mimeType='application/octet-stream'}={}){
    const startedAtMs=Date.now();
    try{const out=await this.client().send(new PutObjectCommand({Bucket:this.bucket,Key:key,ContentType:mimeType,Body:body}));try{this.recordTraffic?.({integration:this.type==='r2'?'r2':'s3',routeKey:`PUT ${this.type==='r2'?'r2':'s3'}://:bucket/:object`,outboundBytes:bodyBytes01201(body),inboundBytes:0,statusCode:Number(out?.$metadata?.httpStatusCode)||200,startedAtMs,finishedAtMs:Date.now(),result:'success'});}catch{}return {etag:str(out?.ETag).replace(/^"|"$/g,'')};}
    catch(error){try{this.recordTraffic?.({integration:this.type==='r2'?'r2':'s3',routeKey:`PUT ${this.type==='r2'?'r2':'s3'}://:bucket/:object`,outboundBytes:bodyBytes01201(body),inboundBytes:0,statusCode:Number(error?.$metadata?.httpStatusCode)||0,startedAtMs,finishedAtMs:Date.now(),result:'failed'});}catch{}throw error;}
  }
