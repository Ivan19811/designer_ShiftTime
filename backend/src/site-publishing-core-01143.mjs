...
export function createSitePublishingService01143({repo,netlify,buildFiles=buildProductionFiles01143,createZip=createZip01143,createTrafficToken=()=>'',now=()=>new Date(),apiProxyTarget='',configured=true}={}){
  ...
  async function publish(scope,userId,builderSiteId,pkg){
    ...
    const ensureTrafficIdentity=async()=>{if(clean(record?.trafficIdentityToken))return record;if(typeof repo.ensureTrafficIdentity!=='function')return record;record=await repo.ensureTrafficIdentity(scope,sid,clean(createTrafficToken()));return record;};
    ...
    if(!clean(record?.netlifySiteId)){
      ...
      await ensureTrafficIdentity();
      files=validateRoot01150(buildFiles(pkg,{canonicalBaseUrl:publicUrl(record),apiProxyTarget,publishedSiteIdentity:{siteId:sid,token:clean(record?.trafficIdentityToken)}}));
    }else{
      await ensureTrafficIdentity();
      files=validateRoot01150(buildFiles(pkg,{canonicalBaseUrl:publicUrl(record),apiProxyTarget,publishedSiteIdentity:{siteId:sid,token:clean(record?.trafficIdentityToken)}}));
    }
