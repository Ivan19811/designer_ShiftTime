...
export function buildPublishedTrafficBridge01203(identity={},apiProxyTarget=''){
  ...
  return `<script data-st-published-traffic="01203" data-st-published-telemetry="01228">(()=>{const token=${safeScriptJson01203(token)},siteId=${safeScriptJson01203(siteId)},apiOrigin=${safeScriptJson01203(apiOrigin)},nativeFetch=globalThis.fetch?.bind(globalThis);globalThis.__ST_PUBLISHED_SITE_IDENTITY__=Object.freeze({stage:'01228',siteId});if(!nativeFetch)return;globalThis.fetch=(input,init={})=>{try{const raw=typeof input==='string'||input instanceof URL?String(input):String(input?.url||''),url=new URL(raw,globalThis.location?.href||'http://localhost/'),allowed=url.pathname.startsWith('/api/')&&(url.origin===globalThis.location?.origin||url.origin===apiOrigin);if(!allowed)return nativeFetch(input,init);const headers=new Headers(input instanceof Request?input.headers:undefined);new Headers(init?.headers||{}).forEach((value,key)=>headers.set(key,value));headers.set('x-st-site-token',token);if(input instanceof Request)return nativeFetch(new Request(input,{...init,headers}));return nativeFetch(input,{...init,headers});}catch{return nativeFetch(input,init);}};if(!globalThis.__ST_PUBLISHED_SITE_PING_01204__){globalThis.__ST_PUBLISHED_SITE_PING_01204__=true;Promise.resolve().then(async()=>{try{let sessionId='';try{const key='st.site.session.01228';sessionId=sessionStorage.getItem(key)||'';if(!sessionId){sessionId='ss_'+(globalThis.crypto?.randomUUID?.()||Math.random().toString(36).slice(2)+Date.now().toString(36));sessionStorage.setItem(key,sessionId);}}catch{sessionId='ss_'+Math.random().toString(36).slice(2)+Date.now().toString(36);}let referrerPath='';try{const ref=document.referrer?new URL(document.referrer,location.href):null;if(ref?.origin===location.origin)referrerPath=ref.pathname+ref.search;}catch{}const response=await globalThis.fetch('/api/v1/public/traffic/ping',{method:'POST',headers:{accept:'application/json','content-type':'application/json'},body:JSON.stringify({sessionId,pagePath:location.pathname+location.search,referrerPath}),cache:'no-store',keepalive:true});const payload=await response.json().catch(()=>({}));if(response.ok)console.info('[ShiftTime 01228] published-site ping success',{siteId,status:response.status,requestId:payload?.requestId||''});else console.warn('[ShiftTime 01228] published-site ping failed',{siteId,status:response.status});}catch(error){console.warn('[ShiftTime 01228] published-site ping failed',{siteId,error:String(error?.message||error||'unknown')});}});}})();</script>`;
}
