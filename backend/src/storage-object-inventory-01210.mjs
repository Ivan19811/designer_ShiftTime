...
export function buildInventorySnapshot01210({scope={},objects=[],referenceState={},providerInfo={},measuredAt=new Date().toISOString()}={}){
  ...
  for(const raw of arr(objects)){
    ...
    items.push({physical:true,provider:object.provider||str(directAsset?.provider||derivative?.provider)||str(providerInfo.provider),bucket:object.bucket||str(directAsset?.bucket||derivative?.bucket)||str(providerInfo.bucket),objectKey:object.objectKey,fileName,extension,sizeBytes:object.sizeBytes,lastModified:object.lastModified,etag:object.etag,storageClass:object.storageClass,mimeType,resourceType,module,modules,accountId,workspaceId:str(directAsset?.workspaceId||derivative?.workspaceId||parentAsset?.workspaceId)||parsed.workspaceId,storeId:str(directAsset?.storeId||derivative?.storeId||parentAsset?.storeId)||parsed.storeId,originSiteId,referencedSiteIds,siteLabels:siteLabels01210(referencedSiteIds,siteById),assetId:referenceAssetId,derivativeId:str(derivative?.id),referenceIds:unique(refs.map(ref=>ref.resourceId)),metadataStatus:str(directAsset?.status||derivative?.status||parentAsset?.status),statusFlags:unique(statusFlags),metadataSource:unique(['r2',directAsset?'media_cloud_assets':'',derivative?'media_asset_derivatives':'',parentAsset?'media_cloud_assets':'',...refs.map(ref=>ref.module)].filter(Boolean))});
  }
...
async function buildAuthorizedSnapshot01210(scope={},options={}){
  ...
  try{
    ...
    return {...snapshot,sites:arr(state.sites),referenceEdges:arr(state.references),source:'provider-prefix-list+postgresql'};
