...
try{
  ...
  if(config.isProduction){
    if(config.corsOrigin==='*')issue(errors,'Production CORS_ORIGIN must be explicit; wildcard is not allowed by 01092 preflight.');
  ...
  console.log(JSON.stringify({ok:errors.length===0,stage:'01092',environment:config.nodeEnv,database:db,migrations:{total:plan.length,pending},warnings,errors},null,2));
