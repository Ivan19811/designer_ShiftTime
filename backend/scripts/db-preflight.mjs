...
try{
  ...
  if(config.isProduction){
    if(config.corsOrigin==='*')issue(errors,'Production CORS_ORIGIN must be explicit; wildcard is not allowed by 01094 preflight.');
  ...
  console.log(JSON.stringify({ok:errors.length===0,stage:'01094',environment:config.nodeEnv,database:db,migrations:{total:plan.length,pending},warnings,errors},null,2));
  if(errors.length)process.exitCode=1;
}catch(e){console.error(JSON.stringify({ok:false,stage:'01094',error:e.message,code:e.code||''},null,2));process.exitCode=1;}finally{await pool.end();}
